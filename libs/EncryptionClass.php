...
class EncryptionClass
{
	...
	function __construct()	{
		...
		$this->scramble1 = '! #$%&()*+,-./0123456789:;<=>?@AZCDEFGIHJKLMNOPQRSTUVWXYB[]^_`azcdefgihjklmnopqrstuvwxyb{|}~';
		$this->scramble2 = 'f^j4?ZgP>dFV=m D<TcS%BHObUe|r:lGK/uCy.Jx)IhQ!#$~(;Lt-R}Ma,NvW+Ynz*AE]ok[2&q1{3`i5w_790p@6s8X';
	...
	//PHP versionm of JScript DES encryption. Use this to encrypt something on the client browser side and decrypt/encrypt it in php
	...
	*/
        ...
        function encryptUIDValue($uid, $saltMin, $saltMax){

          //random length salt
          $hashSaltLen = rand($saltMin, $saltMax);
          $hashSaltStr = '';
          for($j=0; $j < $hashSaltLen; $j++ ){
            $hashSaltStr .= chr(rand(65,90));
          }
          return $hashSaltStr . base64_encode($uid) . $hashSaltLen;
        }


        function decryptUIDValue($hash){

          $saltLen = $hash[strlen($hash)-1];
          $uid = base64_decode(substr($hash, $saltLen, -1));
