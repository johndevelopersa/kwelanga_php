 ...
 */
var dtBack=new Date();
dtBack.setDate(dtBack.getDate()-365);
...
if(typeof(imaxDays) == "undefined")
	imaxDays = 729; //counts only if itype=strict. Enable selection imaxDays from start date
if(typeof(startDay) == "undefined") 
	startDay = dtBack.getDate(); //enable selection from this date
if(typeof(startMonth) == "undefined")
	startMonth = dtBack.getMonth();
if(typeof(startYear) == "undefined")
	startYear = dtBack.getFullYear();
if(typeof(addZero) == "undefined")
	addZero = true; //true|false. Put 0 in front of days&months if <10
if(typeof(offX) == "undefined")
	offX = 50; // x distance from the mouse.
...
var cw = {
    ...
    closeB = cel('span');
    closeB.id = "closeBtn";
    closeB.innerHTML = '<a href="javascript: closeCalendar()"></a>';		
		...
		appendc(brsY, closeB);  //moved close button
		appendc(brsY, backB);
...
function fPopCalendar(param){
	...
	offsets = getScrollXY();
	offsets[0] = jQpageX;  //new mouse cor
	offsets[1] = jQpageY;  //new mouse cor
	
	if ((parseInt($(window).width(),10)-offsets[0])<250) offsets[0] = offsets[0]-250;
	if ((parseInt($(window).height(),10)-offsets[1])<250) offsets[1] = offsets[1]-200;
	...
	document.getElementById('calendar').style.zIndex = "101";
  document.getElementById('calendar').style.left = offsets[0]+ 'px';
  document.getElementById('calendar').style.top = offsets[1]+ 'px';	
	...
	if(folowMouse)
	{
		...
		{
			if (tempX<230) document.getElementById('calendar').style.left = parseInt(tempX)+parseInt(offX)+parseInt(offsets[0]) + 'px';
			else document.getElementById('calendar').style.left = parseInt(tempX)-230+parseInt(offX)+parseInt(offsets[0]) + 'px';
		    document.getElementById('calendar').style.top = parseInt(tempY)+30+parseInt(offY)+parseInt(offsets[1]) + 'px';
		}
		else
		{
			document.getElementById('calendar').style.left = parseInt(tempX)+parseInt(offsets[0])+ 'px';
		    document.getElementById('calendar').style.top = parseInt(tempY)+parseInt(offsets[1])+ 'px';
		}
	}
	*/
...
function fillBackDate(tDay, tMonth, tYear)
{
	...
	elem1.value = vali;
	elem1.onchange();  // added by mark to force onchange event to fire as it doesnt do so if programatically setting the value
