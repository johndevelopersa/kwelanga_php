...
include_once($ROOT.$PHPFOLDER."DAO/stockCountByCategoryDAO.php");
...
$postPRINCIPALNAME = false;
$postUSERNAME = false;
$postDEPOTID = false;
$postDEPOTNAME = false;
$postCATEGORIES = false;
$postOTP = false;
$postPRINCIPLECODE = false;
$postSWITCH = 1;
$postPRODCOUNT = array();
...
switch ($postACTION) {
  ...
  case 'OTP':
    $stockCountByCategoryDAO = new stockCountByCategoryDAO($dbConn);
    $otpUser = $stockCountByCategoryDAO->getOTPUserInfo($postPRINCIPLECODE);
    $smsNumber = '+27'. substr($otpUser[0]["user_cell"], 1, 9);
    $otp = $postOTP;
    $smsMessage = "$postUSERNAME has requested a Stock Adjustment for $postPRINCIPALNAME in the $postDEPOTNAME. The OPT is " . $otp;
