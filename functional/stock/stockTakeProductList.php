...
$productArr = $stockDAO->getStockCountProducts($depotId, $principalId);


//convert product list
if($postPROJSON!=false)
  $jsonArray = @json_decode(html_entity_decode(trim($postPROJSON)),true);
...
foreach($productArr as $pro){
  ...
  if(isset($jsonArray[$pro['product_uid']])){
    ...
    $var = (!$hasVariance)?'<img src="'.$ROOT.$PHPFOLDER.'images/tick_yes.gif" width="12" height="12" alt="no variances">':(($jsonArray[$pro['product_uid']]['v']>0)?('+'.$jsonArray[$pro['product_uid']]['v']):($jsonArray[$pro['product_uid']]['v']));
  }
  ...
  $htmlRow .= '<td nowrap><input type="text" maxlength="10" size="6" value="'.$val.'" name="PRODCOUNT['.$pro['product_uid'].']" class="large-input highlightMe"></td>';
  if($postPROJSON!=false){
