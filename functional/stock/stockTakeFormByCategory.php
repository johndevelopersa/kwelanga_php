...
$LOGGED_IN_USER = $adminDAO->getUserItem($userId);
...
<script type="text/javascript">
  ...
  const Stock = {
    ...
    update: (userId, principalId, username, requiredData) => {
      ...
      const makeStockAdjustment = () => {
        ...
        const params = {
          ...
          "USERID": userId,
          "USERNAME": username,
          "PASSWORD": "<?php echo $LOGGED_IN_USER[0]["password"];?>",
      ...
      const popup_inputs = Popup.GetInfo({
          ...
          mainAction: {
              ...
              action: () => {
                  ...
                  if (true) {
                    // if (otpEntered !== "" && otpEntered == Stock.OTP) {
                    ...
                    Popup.New({
                      ...
                      type: "E",
                      id: "error-update",
                      header: "Incorrect OTP Supplied",
                      message: `Please supply the OTP sent to (${username})`,
                      other: null,
                      mainAction: {
    ...
    sendOTP: (userId, principalId, username, requiredData) => {
      ...
      // make the request to API
      Stock.makeRequest({
        url: '<?php echo $ROOT.$PHPFOLDER ?>functional/stock/stockTakeSubmitNew.php',
     ...
     * @param {{ url: String, params: String }} options
     */
    makeRequestWithReturn: async (options, callback) => {
      // make request
      ...
      if (!response.ok) {
          Popup.New({
              ...
              type: "E",
              id: "success-update",
              header: `Stock Adjustment Failed`,
              message: `Response Captured :: ${JSON.stringify(response)}`,
              other: null,
              mainAction: {
  ...
  function startStockTake(){
    ...
    if(!tc){
      ...
      AjaxRefreshWithResult('ACTION=MODE&SWITCH=1&PRINCIPALID=<?php echo $principalId ?>&DEPOTID=<?php echo $depotId ?>',
                            '<?php echo $ROOT.$PHPFOLDER ?>functional/stock/stockTakeSubmitNew.php',  //is validated again on submit level.
  ...
  function displayProductPrint(imagesCheckBoxId, list){
    const checkBoxEl = document.querySelector(`#${imagesCheckBoxId}`);
    const categories = (selected_categories.length !== 0 || loaded_categories.length === 0)? JSON.stringify(selected_categories) : false;
    ...
    window.open(
      '<?php echo $ROOT.$PHPFOLDER ?>functional/presentations/presentationHandler.php?' + utility.convertObjectToQueryParam(params),
      'ProductList',
      'scrollbars=yes,width=750,height=600,resizable=yes'
    );
  }
  ...
  function displayProductList(json){
    ...
    AjaxRefresh(
      ...
      '<?php echo $ROOT.$PHPFOLDER ?>functional/stock/stockTakeProductListNew.php', // URL to file
