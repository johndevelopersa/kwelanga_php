...
class MYPDF extends TCPDF {
	...
	public function Footer() {
		...
		// Page number
		$this->Cell(0, 10, 'Powered by Kwelanga Solutions (Pty) Ltd', 0, false, 'C', 0, '', 0, false, 'T', 'M');
	}
}
...
if(isset($_GET['IMAGES']) && trim($_GET['IMAGES'] <> 'false')){
  $txt .=  '<th width="100"></th>';
}
$txt .=  '<th >Product Code</th><th width="200">Product Description</th><th width="100">Count</th></tr></thead>';
...
foreach($stockArr as $p){
  ...
  if(isset($_GET['IMAGES']) && trim($_GET['IMAGES'] <> 'false') ){
    ...
    $txt .= '<td height="25"><strong>' . $p['product_code'] .'</strong></td><td width="200">'.
          $p['product_description'] .'</td><td width="100"></td>';
