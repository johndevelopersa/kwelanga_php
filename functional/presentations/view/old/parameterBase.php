...
function f_CHAIN($pfName, $pValue, $pLength, $pLabel, $pRequired, $index) {
	...
	echo "<td style='text-align:top;'>"; BasicSelectElement::getUserPrincipalChainsDD($pfName,$pValue,"N","N",null,null,null,$dbConn,$userId,$principalId); echo "</td>";
	echo "</tr>";
...
function f_CHAINMULT($pfName, $pValue, $pLength, $pLabel, $pRequired, $index) {
	...
	$mfC = $storeDAO->getAllPrincipalChainsForUser($userId, $principalId);
...
function f_STORE($pfName, $pValue, $pLength, $pLabel, $pRequired, $index) {
	...
	echo "<td style='text-align:top;'>"; BasicSelectElement::getUserPrincipalStoresDD($pfName,$pValue,"N","N",null,null,null,"",$dbConn,$userId,$principalId); echo "</td>";
	echo "</tr>";
...
function f_SALESREPMULT($pfName, $pValue, $pLength, $pLabel, $pRequired, $index) {
	...
	foreach ($mfC as $row) {
		...
		if ((in_array($value,$defaults)) || (in_array("*",$defaults))) $CHECKED=" CHECKED "; else $CHECKED="";
		echo "<tr>";
		echo "<td><input name='".$pfName."' type='checkbox' value='".$value."' ".$CHECKED." /></td><td>".$row['rep_code'] . ' - ' . $row['first_name'] . ' ' . $row['surname']."</td>";
		echo "</tr>";
	}
	echo "</table>";
	echo "</div>";
...
if (($postSCHEDULER!="Y") && ($postGENERALUSAGE!="Y")) {
  echo '<TR class="'.GUICommonUtils::styleEO($class).'">';
    ...
    echo '<TD>';
      BasicSelectElement::getReportOutputTypesDD('f_OUTPUTTYPE', SCD_OT_HTML, "N", "N", null, null, null);
    echo '</TD>';
  echo '</TR>';
