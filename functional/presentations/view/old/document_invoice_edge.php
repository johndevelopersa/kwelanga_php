  ...
  <tr>
    ...
    <td>
      ...
      <div  align="center" style="margin:30px 0px;">
        <h1><span id="printedtxt"></span> Goods Dispatched Note</h1>
        ...
        <table border="0" cellpadding="6" cellspacing="0" width="100%" class="grid">
          ...
          <tr>
            <td>Order Date:</td>
            <td width="230"><strong><?php echo $mfT[0]['order_date'] . ' ' . CommonUtils::convertUserTime($mfT[0]['processed_time']) ?></strong></td>
            <td width="180">Ordered By:</td>
            <td width="150"><strong>
            <?php
            ...
            $sql = "SELECT
                      d.from_alias
                       FROM distribution d
                       WHERE
                      DATE(d.queued_date) BETWEEN  ('".$mfT[0]['order_date']."' - INTERVAL 6 DAY) AND ('".$mfT[0]['order_date']."' + INTERVAL 6 DAY)
                      AND d.source_identifier like CONCAT('SEQ:',ABS('".$mfT[0]['customer_order_number']."'),'%')
                      LIMIT 1";

              $row = $dbConn->dbGetAll($sql);
              if(isset($row[0]['from_alias'])){
                echo $row[0]['from_alias'];
              }
        ...
        <table border="0" cellpadding="0" cellspacing="0" width="100%" >
          <tr>
           ...
           <td valign="bottom"><strong> Customer:</td><td  class="bordUnderline">&nbsp;</td>
          </tr><tr>
           <td valign="bottom"><strong> Waybill No.</td><td  class="bordUnderline">&nbsp;</td>
          </tr>
        </table>
