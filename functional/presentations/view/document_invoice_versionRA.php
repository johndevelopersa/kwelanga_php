     ...
     <table style="border-collapse:collapse; width:98%">
         <tr>
             ...
             <?php    $document_type = '12'; 
                         ...
                         <?php } else { ?>
                                    <td class="dc" style="text-align:left;">INVOICE</td>
                         <?php } ?>   
                      <td width="50%;" >&nbsp;</td>
         ...
         <tr>
          <?php 
           	   if($mfT[0]['delivery_instructions'] <> NULL) { 
           	   	  $insfact = strlen($mfT[0]['delivery_instructions']) / 2;
                  $loop = 1;    	   	  
           	   	  $insert  = $insfact * $loop;
           	   	  $delstring = '';
           	   	  for ($x = 0; $x <= strlen($mfT[0]['delivery_instructions']); $x++) {
           	   	  	  if(substr($mfT[0]['delivery_instructions'],$x,1) == ' ' && $x >= $insert) {
                         $loop++;    	   	  
           	   	         $insert  = $insfact * $loop;
           	   	         $delstring = $delstring . '+';
           	   	  	  } else {
           	   	  	     $delstring = $delstring . substr($mfT[0]['delivery_instructions'],$x,1);	
           	   	  	  }
                   }
           	   	?>
                <td class="dc3" colspan="2"; style="text-align:left;" nowrap ><span style="font-weight:bold ;">Special Instructions</span>&nbsp;&nbsp;<span style="font-weight:normal;"><?php echo (str_replace(chr(43),"<br>", $delstring))?></span></td>
           <?php } ?>         
         </tr>
...
<script type="text/javascript">
  function emailDoc1() {
      window.location="<?php echo $ROOT.$PHPFOLDER ?>functional/presentations/CreateAndSavePDF.php?DOCMASTID=<?php echo $docmastId;?>&USERID=<?php echo $userId;?>&PRINCIPALID=<?php echo $principalId;?>&OUTPUTTYP=<?php echo 'pdfSelf'.trim($document_type). substr($mfT[0]['document_number'],0,8);?>&PSMUID=<?php echo ('');?>&PRINNAM=<?php echo ($mfT[0]['principal_name']);?>&TEMPLAT=<?php echo 'document_invoice_versionRA.php';?>";
  }
  function emailDoc2() {
  	  window.location="<?php echo $ROOT.$PHPFOLDER ?>functional/presentations/CreateAndSavePDF.php?DOCMASTID=<?php echo $docmastId;?>&USERID=<?php echo $userId;?>&PRINCIPALID=<?php echo $principalId;?>&OUTPUTTYP=<?php echo 'pdfCust'.trim($document_type). substr($mfT[0]['document_number'],0,8);?>&PSMUID=<?php echo($mfT[0]['psm_uid']);?>&PRINNAM=<?php echo ($mfT[0]['principal_name']);?>&TEMPLAT=<?php echo 'document_invoice_versionRA.php';?>";
}  
</script> 