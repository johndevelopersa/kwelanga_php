...
<HTML>
    ...
    if(isset($_POST['UPDTRAN'])) { 
        ...
        if (isset($_POST["TUID"]))    $postTUID    = test_input($_POST["TUID"]);    else $postTUID      = '';
    	  ...
    	  if (isset($_POST["TREG"])) $postNREG = test_input($_POST["TREG"]); else $postNSTATUS   = '';  

        if(strlen($postTNAME) > 3) {
              $TripsheetDAO = new TripsheetDAO($dbConn);
              $errorTO = $TripsheetDAO->updateTransporter($postTUID, $postTNAME, $postNSTATUS, $postNREG);
    ...
    if(isset($_POST['SELMOD'])) {
    	   ...
    	   $TransportUid = trim(substr($_POST["SELMOD"],0,strpos($_POST["SELMOD"],"-")));
    	   
    	   $TripsheetDAO = new TripsheetDAO($dbConn);
         $TransporterDet = $TripsheetDAO->GetTransporterReg($TransportUid);
        
         $RegNum = $TransporterDet[0]['vehicle_reg'];
       ...
       <center>
          <form name='Maintain Transporter' method=post action='' onload='setFocusTPselect()'>
             <table width="720"; style="border:none">
                 ...
                 <?php
                 // echo trim(substr($_POST["SELMOD"],strpos($_POST["SELMOD"],"-") + 1,50));
                  //NELLWYN/ ANDILE - Bakkie
                 if($_POST['CURSTAT'] == "D") {
                    $dCheck = 'CHECKED';
                    $aCheck = '';
                 } else {
                 ...
                 <tr class="<?php echo GUICommonUtils::styleEO($class); ?>">
                     <td Colspan="1" >&nbsp</td>
                     <td class="head1" colspan="1" style="text-align:left; padding: 0 0px 0 0px ; ">Transporter Reg </td>
                     
                     <td Colspan="2" ><INPUT TYPE="TEXT" size="40" name="TREG" id="TREG" value= '<?php echo trim($RegNum); ?>'></td>
                 </tr>
