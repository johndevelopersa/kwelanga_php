...
class EzTradeInvoicedInit extends extractController {

  private $principalUid = 319; //uid of principal extract.
  private $filename = 'PAS319[@FSEQ].txt';  //main controller will build full filename with seq. for us.
  private $crnFilename = 'CRN319[@FSEQ].txt';  //credit note filename
  ...
  public function generateOutput(){
    ...
    if (sizeof($psms)>0) {
      $sfvals_PA = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 392, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
    ...
    $depotTrans = array(
      ...
      185 =>   'BRA', //Brand WH
      187 =>   'BEL', //BELVIGLO
      193 =>   '003', // CLEAR WORLD EASTERN CAPE 
      276 =>   '002' // CLEAR WORLD KZN
      );
              ...
              if(abs($d['document_qty'])>0){
                ...
                if($d['non_stock_item'] == 'Y') {
                	...
                	$pastelType = '6';                	
                	 ...
                	 $pastelType = '4';
                   if ($d['invoice_total'] == 0 && $d['document_qty'] > 0) {
