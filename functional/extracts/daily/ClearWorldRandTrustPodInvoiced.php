...
class ClearWorldRandTrustPodInvoiced {
...
class ClearWorldRandTrustPodInvoicedInit extends extractController {
	
  private $principalUid = 216; //uid of principal extract.
  private $filename = 'RAND_TRUST_POD_[@FSEQ].csv'; //main controller will build full filename with seq. for us.
  ...
  public function generateOutput(){
    ...
    if (!$this->skipInsert) {
      ...
      $rTO = $this->postExtractDAO->queueAllInvoiced($this->principalUid, $recipientUId, $inclCancelled = false, $dtarr = array(DT_ORDINV, DT_ORDINV_ZERO_PRICE), $statusarr=array(DST_DELIVERED_POD_OK, DST_DIRTY_POD));  //use the loaded receipientUID and not the notification type... *** same as document confirmations***
    ...
    $fmtArr    = array();    
    $fmtArr[]  = 'Completed (outstanding) P.O.D List            PLEASE NOTE: NOT FOR IMPORTING';
    $dataArr[] = join(',',$fmtArr);  
    
    $fmtArr    = array();    
    $fmtArr[]  = 'DATE:     Attention: Buying Department';
    $dataArr[] = join(',',$fmtArr);  

    $fmtArr    = array();    
    $fmtArr[]  = 'Client Name =Clear World';
    $fmtArr[]  = '';	
    $fmtArr[]  = '';
    $fmtArr[]  = '';
    $fmtArr[]  = '';
    ...
    if(empty($sfvals_PA[$ord[0]["principal_store_uid"]]['value'])) {  //has special field and not blank...
      ...
      $postingDistributionTO->subject = (($type=='i')?$this->getTemplateRandPodInvoiceSubject():$this->getTemplateRandPODSubject()); //might have no filename if all errors therefore don't display on subject line...
      ...
      if (sizeof($errorSEUIdArr) > 0) {
        $bIResult = $this->postBIDAO->setSmartEventStatusBulk(implode(",", $errorSEUIdArr), "324", "", FLAG_ERRORTO_NOT_EXTRACTED);
      }
