...
class HoneyfieldsInvoicedInit extends extractController {

  private $principalUid = 305; //uid of principal extract.
  private $filename = 'PAS305[@FSEQ].csv';  //main controller will build full filename with seq. for us.
  private $crnFilename = 'CRN305[@FSEQ].csv';  //credit note filename
  ...
  public function generateOutput(){
    ...
    if (sizeof($psms)>0) {
      $sfvals_PA = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 343, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
    }
    ...
    $successCount = 0;
      ...
      foreach($orders as $ord){
        ...
        if(empty($sfvals_PA[$ord[0]["principal_store_uid"]]['value'])) {  //has no special field and/or blank...
          ...
          if($type == 'i'){
            ...
            if($ord[0]["source_document_number"] <> '') {
      ...
      if(count($successSEUIdArr)>0){
        ...
        $fp = file_put_contents(DIR_DATA_NON_FTP_FROM. 'ftp/honeyfields/' . $seqFilename, $data);
        if($fp != strlen($data)){
          BroadcastingUtils::sendAlertEmail("System Error", "Failed to create extract file!", "Y");
          $this->errorTO->type = FLAG_ERRORTO_ERROR;
