...
class SupaGinjaInvoicedCheckInit extends extractController {

  private $principalUid = 408; //uid of principal extract.
  private $filename = 'PAS408[@FSEQ].txt';  //main controller will build full filename with seq. for us.
  private $crnFilename = 'CRN408[@FSEQ].txt';  //credit note filename
  ...
  public function generateOutput(){
    ...
    if (sizeof($psms)>0) {
      $sfvals_PA = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 551, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
      $sfvals_SA = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 552, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
   }
    ...
    foreach($orders as $ord){
        ...
        /*-------------------------------------------------*/
        	...
        	$regionArr = (isset($depotTrans[$ord[0]['branch_code']])) ? $depotTrans[$ord[0]['branch_code']] : false;
          ...
          if((empty($sfvals_PA[$ord[0]["principal_store_uid"]]['value'])) || (empty($sfvals_SA[$ord[0]["principal_store_uid"]]['value']))) {  //has no special field and/or blank...
            ...
            if ($ord[0]['off_invoice_discount'] <> '') {
                     ...
                     $hDisc = number_format($ord[0]['off_invoice_discount'], 2, '.', '') ;
            }
            ...
            if($type == 'i'){
              $rowArr[] = '"' . str_pad(substr($ord[0]['document_number'],-8), 8, 0, STR_PAD_LEFT) . '"';
            } else {
              $rowArr[] = '"' . str_pad(substr($ord[0]['alternate_document_number'],-8), 8, 0, STR_PAD_LEFT) . '"';
            }
