...
class IlbHeliosInvoicedInit extends extractController {

  private $principalUid = 340; //uid of principal extract.
  private $filename = 'PAS340[@FSEQ].txt';  //main controller will build full filename with seq. for us.
  private $crnFilename = 'CRN340[@FSEQ].txt';  //credit note filename
  ...
  public function generateOutput(){
    ...
    if (sizeof($psms)>0) {
      $sfvals_PA = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 412, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
      $sfvals_SA = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 413, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
    }

    $depotTrans = array(
      190   =>   'PE1', // Deliver IT PE
      186   =>   'CT1', // Deliver IT Capetown
      202   =>   'DB1', // Deliver IT KZN         
      195   =>   'JB1' // Loginet
      );
