...
class KwelangaPastelInvoicedInit extends extractController {

  private $principalUid = 309; //uid of principal extract.
  private $filename = 'PAS309[@FSEQ].csv';  //main controller will build full filename with seq. for us.
  private $crnFilename = 'CRN309[@FSEQ].csv';  //credit note filename
  ...
  public function generateOutput(){
    ...
    if (!$this->skipInsert) {
      ...
      $rTO = $this->postExtractDAO->queueAllCreditsAndDebits($this->principalUid, $recipientUId, array(DT_CREDITNOTE, DT_MCREDIT_OTHER));  //use the loaded receipientUID and not the notification type... *** same as document confirmations***
      if ($rTO->type != FLAG_ERRORTO_SUCCESS) {
    ...
    if (sizeof($psms)>0) {
      $sfvals_PA = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 354, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
    }
    ...
    $successCount = 0;
      ...
      foreach($orders as $ord){
        ...
        if(empty($sfvals_PA[$ord[0]["principal_store_uid"]]['value'])) {  //has no special field and/or blank...
          ...
          if($type == 'i'){
            $rowArr[] = '"'.date("d/m/Y", strtotime($ord[0]["invoice_date"])).'"';  //DATE (DD/MM/YYYY)
            if ($ord[0]["invoice_date"] < '2018-04-01') {
                   $oldVATRate = "T";	
            }    
          } else {
            $rowArr[] = '"'.date("d/m/Y", strtotime($ord[0]["order_date"])).'"';  //DATE (DD/MM/YYYY)
          ...
          foreach($ord as $d){ //detail rows.
            ...
            if(abs($d['document_qty'])>0){
              ...
              if($type == 'i'){
                ...
                $detArr[] = '" "';  //UNIT - CHAR
                  if($oldVATRate == "T") {
                  	$detArr[] = (substr($d['vat_rate'],0,2)=='14')?01:2;
                  } else {
      ...
      if (sizeof($errorSEUIdArr) > 0) {
        $bIResult = $this->postBIDAO->setSmartEventStatusBulk(implode(",", $errorSEUIdArr), "354", "", FLAG_ERRORTO_ERROR);
        if ($bIResult->type != FLAG_ERRORTO_SUCCESS) {
          $this->errorTO->type = FLAG_ERRORTO_ERROR;
