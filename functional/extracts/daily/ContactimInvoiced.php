...
class ContactimInvoicedInit extends extractController {


  private $principalUid = 70; //uid of principal extract.
  private $invFilename = 'IN[@REGION]84[@FSEQ].TXT';  //main controller will build full filename with seq. for us.
  private $crnFilename = 'CR[@REGION]84[@FSEQ].TXT';  //main controller will build full filename with seq. for us.
  private $debFilename = 'DN[@REGION]84[@FSEQ].TXT';  //main controller will build full filename with seq. for us.
            ...
            if(empty($ord[0]['old_account'])){
              ...
              foreach($ord as $d){ //detail rows => rows per order.
                ...
                if (abs($d['document_qty']) <> 0) {
                   ...
                   $data .= str_pad(substr(abs($ord[0]['document_number']),-8), 8, '0', STR_PAD_LEFT); //grv number for credits/debits.
                   ...
                   if($type != 'i'){
                     $data .= str_pad(substr(abs($ord[0]['source_document_number']),-8), 8, '0', STR_PAD_LEFT); //grv number for credits/debits.
                   }
                   $data .= str_pad('', 395, ' ') . "\r\n";
                }
              } //eo detail
