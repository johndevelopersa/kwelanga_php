...
class NicolettaInvoicedInit extends extractController {

  private $principalUid = 218; //uid of principal extract.
  private $filename = 'PAS218[@FSEQ].csv';  //main controller will build full filename with seq. for us.
  private $crnFilename = 'CRN218[@FSEQ].csv';  //credit note filename
  ...
  public function generateOutput(){
    ...
    if (sizeof($psms)>0) {
      $sfvals_PA = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 245, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
      $sfvals_SA = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 289, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
    }
     
     $depotTrans = array(
      185 =>   'GY', //Brand WH
      260 =>   'GY', //Yaels Warehouse
      104 =>   'LB',  //L&B
      );
    ...
    $successCount = 0;
      ...
      foreach($orders as $ord){
        ...
        if((empty($sfvals_PA[$ord[0]["principal_store_uid"]]['value'])) || (empty($sfvals_SA[$ord[0]["principal_store_uid"]]['value']))) {  //has no special field and/or blank...
          ...
          //period
          $period = '00';
          $idate  = date("Ymd", strtotime($ord[0]["invoice_date"]));
          switch ($idate) {
            case  $idate >= '20210102' && $score <= '20210129': 
              $period = 11;
              break;
            case  $idate >= '20210130' && $score <= '20210226': 
              $period = 12;
              break;
            case '03':
              $period = 1;
            ...
            case '06':
              $period = 4;
              break;
            case $idate >= '20200704' && $score <= '20200731': 
              $period = 5;
              break;
            case $idate >= '20200801' && $score <= '20200828': 
              $period = 6;
              break;
            case $idate >= '20200829' && $score <= '20201002': 
              $period = 7;
              break;
            case $idate >= '20201003' && $score <= '20201030':
              $period = 8;
              break;
            case $idate >= '20201031' && $score <= '20201127': 
              $period = 9;
              break;
            case $idate >= '20201128' && $score <= '20210101': 
              $period = 10;
              break;
          }
          ...
          if($type == 'i'){
            $rowArr[] = '"IU' . str_pad(substr($ord[0]['document_number'],-6), 6, 0, STR_PAD_LEFT) . '"';
          } else {
            $rowArr[] = '"CU' . str_pad(substr($ord[0]['alternate_document_number'],-6), 6, 0, STR_PAD_LEFT) . '"';
          }
...
//            }    
      ...
      if (sizeof($errorSEUIdArr) > 0) {
        $bIResult = $this->postBIDAO->setSmartEventStatusBulk(implode(",", $errorSEUIdArr), "245,289", "", FLAG_ERRORTO_ERROR);
        if ($bIResult->type != FLAG_ERRORTO_SUCCESS) {
          $this->errorTO->type = FLAG_ERRORTO_ERROR;
