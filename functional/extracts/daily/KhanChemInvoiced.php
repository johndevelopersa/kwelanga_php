...
class KhanChemInvoicedInit extends extractController {

  private $principalUid = 459; //uid of principal extract.
  
  private $filename    = 'PAS459[@FSEQ].txt';  //main controller will build full filename with seq. for us.
  ...
  public function generateOutput(){
    ...
    $recipientUId = $reArr[0]['uid'];
        ...
        if (sizeof($psms)>0) {
          $sfvals_PA = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 642, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
            ...
            /*-------------------------------------------------*/
              ...
              if((empty($sfvals_PA[$ord[0]["principal_store_uid"]]['value'])) ) {  //has no special field and/or blank...
                ...
                if($type == 'i'){
                  ...
                  $rowArr[] = '"'. str_pad(substr($ord[0]['document_number'],-6), 6, 6, STR_PAD_LEFT) . '"';
                }
