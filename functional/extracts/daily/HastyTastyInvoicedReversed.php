...
class HastyTastyInvoicedReversed {
...
class HastyTastyInvoicedReversedInit extends extractController {

  private $principalUid = 71; //uid of principal extract.
  private $filename = 'CRN71[@FSEQ].txt';  //main controller will build full filename with seq. for us.
  private $crnFilename = 'DEB71[@FSEQ].txt';  //credit note filename
...
/*    if (!$this->skipInsert) {
      ...
      $rTO = $this->postExtractDAO->queueAllInvoiced($this->principalUid, $recipientUId, 
                                                     $inclCancelled = false, 
                                                     $dtarr = array(DT_ORDINV, DT_ORDINV_ZERO_PRICE),
                                                     $dsArr = array(DST_DELIVERED_POD_OK, DST_DIRTY_POD, DST_POD_SCANNED));  //use the loaded receipientUID and not the notification type... *** same as document confirmations***
...
*/    
    ...
    if (sizeof($psms)>0) {
      $sfvals_PA = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 355, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
      $sfvals_SA = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 544, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
   }
    $depotTrans = array(
      229     =>  '005',    //	Hasty Tasty
      104     =>  '001',    //	LOGINET
      9       =>  '008',    //	Megamore East London
      8       =>  '003',    //	Megamore George
      120     =>  '002',    //	Megamore Port Elizabeth
      372     =>  '007',    //	SGX LOGISTICS
      191     =>  '009',    //	Target Distribution
      202     =>  '006'     //	TK Logistics and Warehousing
      );
    ...
    foreach($orders as $ord){
        ...
        if(empty($sfvals_PA[$ord[0]["principal_store_uid"]]['value'])) {  //has no special field and/or blank...
            ...
            echo str_pad(substr($ord[0]['invoice_number'],-6), 6, 0, STR_PAD_LEFT);
            echo "<br>";
            ...
            if($type == 'c'){
            	...
            	echo "JJ";
            	echo "<br>";
              $rowArr[] = '"CR' . str_pad(substr($ord[0]['invoice_number'],-6), 6, 0, STR_PAD_LEFT) . '"';
            } else {
              $rowArr[] = '"DB' . str_pad(substr($ord[0]['alternate_document_number'],-6), 6, 0, STR_PAD_LEFT) . '"';
            }
            ...
            if($type == 'c'){
              ...
              $rowArr[] = '"'.trim(str_replace(array('"',"'"),array('',''),$ord[0]["branch_code"])).'"';
            } else {
            ...
            $rowArr[] = '"0"';  //discount
            if($type == 'c'){
                 $rowArr[] = '"'.trim(str_replace(array('"',"'"),array('',''),$ord[0]["customer_order_number"])).'"';  //MESSAGE - CHAR
            } else {
                if(trim($ord[0]['claim_number']) <> '') {
                  $rowArr[] = '"' . $ord[0]['claim_number'] . '"';
                } else {
                  $rowArr[] = '"NO CLAIM Num"';
                }    
            }
            ...
            foreach($ord as $d){ //detail rows.
              ...
              if(abs($d['document_qty'])<>0){
                ...
                if($type == 'c'){
                  ...
                  $detArr[] = (substr($d['vat_rate'],0,2)=='15')?20:2;
                  ...
                  $detArr[] = number_format(abs(round(($d['selling_price']*VAL_VAT_RATE_ADD), 2)), 2, '.', '');  //INCLUSIVE PRICE - NUM	
                  $detArr[] = '" "';  //UNIT - CHAR
                  $detArr[] = (substr($d['vat_rate'],0,2)==VAL_VAT_RATE_TBLSTD)?20:2;
                  $detArr[] = '3'; //DISCOUNT TYPE
                  $detArr[] = (abs($d['selling_price'])>0 && abs($d['discount_value'])>0) ?  number_format(round($d['discount_value'] / $d['selling_price'] * 100, 2), 2, '', '') : 0; //DISCOUNT %  //stored value : discount_value backwards calculation.
                }
                $pastelType = '4';
      ...
      if(count($successSEUIdArr)>0){
        ...
        //determine seq.
        $seqFilename = $this->setFilenameFSEQ((($type=='c')?$this->filename:$this->crnFilename), $this->principalUid, false, 3, self::setFilenameFSEQ_LenType_PAD);
      ...
      if (sizeof($errorSEUIdArr) > 0) {
        $bIResult = $this->postBIDAO->setSmartEventStatusBulk(implode(",", $errorSEUIdArr), "521", "", FLAG_ERRORTO_ERROR);
        if ($bIResult->type != FLAG_ERRORTO_SUCCESS) {
          $this->errorTO->type = FLAG_ERRORTO_ERROR;
