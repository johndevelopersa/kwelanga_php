...
class NaturalHerbInvoicedInit extends extractController {


  private $principalUid = 293; //uid of principal extract.
  ...
  public function generateOutput(){
    ...
    $folder = $this->principalUid . '_natherb'; // . explode(' ',  strtolower($pArr[0]['principal_name']))[0]; //folder replaced with principal id + first WORD of principal.
    ...
    if (sizeof($psms)>0) {
          if ($newcodes == 'Y') {
          	...
          	190 => "I3",  // Deliver IT
            104 => "I6" , // DC CHS - L&B
            195 => "I5"); // DC CHS - Loginet
            $sfvals = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 338, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
          } else {
          	$depotTrans = array(
            218 => "F2", // DC CHS - Imperial FST
          	219 => "F3"); // DC CHS - Imperial WC CHS
            $sfvals = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 334, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
          }
    }
          ...
          foreach($hr as $d){
            ...
            if(abs($d['document_qty'])==0){
              ...
              $data .=  "     <StockLine>\n".
                        ...
                        "       <StockCode>{$d['alt_code']}</StockCode>\n". //product code
