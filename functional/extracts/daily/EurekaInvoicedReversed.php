...
class EurekaInvoicedReversedInit extends extractController {

  private $principalUid = 403; //uid of principal extract.
  private $filename = 'CRN403[@FSEQ].txt';  //main controller will build full filename with seq. for us.
  private $crnFilename = 'DEB403[@FSEQ].txt';  //credit note filename
...
*/    
    ...
    if (sizeof($psms)>0) {
      $sfvals_PA = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 543, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
    ...
    foreach($orders as $ord){
        ...
        if(empty($sfvals_PA[$ord[0]["principal_store_uid"]]['value'])) {  //has no special field and/or blank...
            ...
            echo str_pad(substr($ord[0]['document_number'],-6), 6, 0, STR_PAD_LEFT);
            echo "<br>";
