...
class ITEDocumentInit extends extractController {

  private $principalUid = 342; //uid of principal extract.
  private $filename    = 'INV_342[@FSEQ].csv';  //main controller will build full filename with seq. for us.
  private $crnFilename = 'INV_342[@FSEQ].csv';  //credit note filename
  public function generateOutput(){
    ...
    if (sizeof($psms)>0) {
        $sfvals_pa   = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 394, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
        $sfvals_area = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 393, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
        $sfvals_rep  = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 395, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
    }
    ...
    foreach($grpDocs as $type => $orders){
       ...
       $depotTrans = array(286   =>   'ITE-033-FIN',   //  L&B-Finished Goods
                           285   =>   'ITE-025-FIN',    //  PE
                           283   =>   'ITE-031-FIN',    //  Nelspruit
                           282   =>   'ITE-026-FIN',        //  KZN
                           271   =>   'ITE-020-FIN');   //  EMIT LOGISTICS            
   ...
   //          	print_r($ord);
                      ...
                      $rowArr[] = '0';
                      if(in_array(trim($ord[0]["principal_store_uid"]), $cfa)) {$rowArr[] = str_replace('ITE', 'CFA',$d['product_code']);} else {$rowArr[] = $d['product_code'];}
                      if(in_array(trim($ord[0]["principal_store_uid"]), $cfa)) {$rowArr[] = str_replace('ITE', 'CFA',$regionArr);} else {$rowArr[] = $regionArr;}                                         // 11
                      $rowArr[] = $d['product_description'];                                                            // 12
                      ...
                      $rowArr[] = $repCode;                                                                             // 20
                      if(in_array(trim($ord[0]["principal_store_uid"]), $cfa)) {$rowArr[] = '24';} else {$rowArr[] = '12';} //21
                      $rowArr[] = '';                                                                                       //22
