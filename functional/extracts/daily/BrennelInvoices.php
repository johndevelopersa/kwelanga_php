...
class BrennelInvoicesInit extends extractController {

  private $principalUid = 424; //uid of principal extract.
  private $filename    = 'INV_424[@FSEQ].csv';  //main controller will build full filename with seq. for us.
  private $crnFilename = 'INV_424[@FSEQ].csv';  //credit note filename
  public function generateOutput(){
    ...
    $reArr = $this->bIDAO->getNotificationRecipients($this->principalUid, 
                                                     NT_DAILY_EXTRACT_ALTCUSTOM3);
    if (count($reArr)==0) {
    ...
    if (!$this->skipInsert) {
      ...
      $rTO = $this->postExtractDAO->queueAllInvoiced($this->principalUid, 
                                                     ...
                                                     $dataSource=false,
                                                     $capturedBy=false,
                                                     $depotUId = $depotList);  //use the loaded receipientUID and not the notification type... *** same as document confirmations***
    ...
    if (sizeof($psms)>0) {
        $sfvals_pa   = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 581, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
    }
