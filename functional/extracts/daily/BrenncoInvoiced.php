...
class BrenncoInvoicedInit extends extractController {
  ...
  public function generateOutput(){
    ...
    foreach($seDocs as $k=>$r){
      ...
      $grpDocs[$index][$r['depot_code']][$r['dm_uid']][] = $r;
    ...
    if (sizeof($psms)>0) {
      ...
      $sfBrennerRegion = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 345, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
      ...
      $sfBrennerAcc = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 344, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
    ...
    foreach($grpDocs as $type => $depotOrders){

      foreach($depotOrders as $depotId => $docArr){  //within each type is each depot -> create file per depot.
          ...
          if($buyerRef == '40009124' || $buyerRef == '30006184' || $buyerRef == '90006095' || $buyerRef == '1000002810'){
            ...
            if(substr(trim($hr[0]['product_code']),0,1) == '9' && substr($hr[0]['product_code'],0,3)!='980'){
              ...
              //DEFAULT TO BRENNCO ACOCUNT : CAPTURE AND GPH
                   ...
                   if ($hr[0]['depot_uid'] == 134) {
            	     	   $chosenSFVal = $sfBrennerAcc;
            	     } else {
          ...
          if(
            ...
            if ($hr[0]['depot_uid'] == 3) {
                   ...
                   $regionCode = '405';
            	     ...
            	     if ($hr[0]['depot_uid'] == 134) {
            	     	   $regionCode = trim($sfBrennerRegion[$hr[0]["principal_store_uid"]]['value']);
            	     } else {
            ...
            if($type == 'c'){
              ...
              if($hr[0]['depot_uid'] == 188) {
            	     $dataH[] = $hr[0]['invoice_number']; //doc number for inv and cre.	
            	} else {
            ...
            if($type == 'c'){
              ...
              $dataH[] = ' '; //blank
              $dataH[] = $regionCode . '-' . (!empty($hr[0]['invoice_number']) ? str_pad(substr($hr[0]['invoice_number'],-6), 6, '0', STR_PAD_LEFT) : str_pad(substr(abs($hr[0]['document_number']),-6), 6, '0', STR_PAD_LEFT)); //blank : " + BRENREG + "-" +DDNO
            }
            ...
            foreach($hr as $d){ //detail rows.
              if(abs($d['document_qty'])>0){  //invoice qty greater then zero.
                ...
                $dataD[] = abs(number_format(round($d['net_price'], 2), 2, '.', ''));
