...
class GQTissueInvoicedInit extends extractController {

  private $principalUid = 193; //uid of principal extract.
  private $filename = 'PAS193[@REGION][@FSEQ].csv';  //main controller will build full filename with seq. for us.
  private $crnFilename = 'CRN193[@REGION][@FSEQ].csv';  //credit note filename
  ...
  public function generateOutput(){
    ...
    $seDocs = $this->extractDAO->getDailyExtractInvoicedOrders($this->principalUid, $recipientUId, $orderBy="if(a.document_type_uid in (".DT_CREDITNOTE.",".DT_DEBITNOTE."),a.alternate_document_number,b.invoice_number), c.uid");
    ...
    foreach($seDocs as $k=>$r){
      ...
      $grpDocs[$r['depot_uid']][$type][$r['dm_uid']][] = $r;
      $psms[$r["principal_store_uid"]] = $r["principal_store_uid"];
    }

    // get special field values for all stores in above docs
    if (sizeof($psms)>0) {
      $sfvals_PA = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 225, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
      $sfvals_SA = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 227, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
    }

    foreach($grpDocs as $depot => $dptOrders){

      $locationCode = "";
      switch ($depot) {
        case 7:
          $locationCode = "TEL"; // Turner Brothers
          break;
        case 104:
          $locationCode = "LNB"; //L&B
          break;
        case 190:
          $locationCode = "DPE"; //Deliver it
          break;
        case 185:
          $locationCode = "BWH"; //Brand Warehous
          break;
        case 188:
          $locationCode = "RWC"; // Retail Logistic
          break;
        case 149:
          $locationCode = "TSM"; // Trend
          break;
        case 228:
          $locationCode = "WPE"; // WOLJO 
          break;
        case 235:
          $locationCode = "WGR"; // WOLJO Karoo
          break;
        default:
          $locationCode = "NOT CONFIGURED";
      }

      foreach($dptOrders as $type => $orders){
        ...
        foreach($orders as $ord){
          ...
          if((empty($sfvals_PA[$ord[0]["principal_store_uid"]]['value'])) || (empty($sfvals_SA[$ord[0]["principal_store_uid"]]['value']))) {  //has no special field and/or blank...
            ...
            if($type == 'i'){
              $rowArr[] = '"IA' . str_pad(substr($ord[0]['invoice_number'],-6), 6, 0, STR_PAD_LEFT) . '"';
            } else {
            	if($locationCode == "RWC") {
             	   $rowArr[] = '"ICB' . abs($ord[0]['alternate_document_number']) . '"';
            }	elseif($locationCode == "WGR") {
             	   $rowArr[] = '"ICB' . abs($ord[0]['alternate_document_number']) . '"';
           	} else {	
            	   $rowArr[] = '"IC' . abs($ord[0]['alternate_document_number']) . '"';
              }
            }
            ...
            if($type == 'i'){
              ...
              $rowArr[] = '"'.trim(str_replace(array('"',"'"),array('',''),$ord[0]["invoice_number"])).'"';
            ...
            foreach($ord as $d){ //detail rows.
              ...
              if(abs($d['document_qty'])>0){
                ...
                $detArr[] = '" "';  //UNIT - CHAR
                $detArr[] = (substr($d['vat_rate'],0,2)=='15')?15:02;
        ...
        //determine seq.
          ...
          $seqFilename = str_replace("[@REGION]",$locationCode,$seqFilename);
