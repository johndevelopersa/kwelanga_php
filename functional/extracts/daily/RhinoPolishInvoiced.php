...
class RhinoPolishInvoicedInit extends extractController {

  private $principalUid = 357; //uid of principal extract.
  private $filename = 'PAS357[@FSEQ].csv';  //main controller will build full filename with seq. for us.
  private $crnFilename = 'CRN357[@FSEQ].csv';  //credit note filename
  ...
  public function generateOutput(){
    ...
    if (sizeof($psms)>0) {
      $sfvals_PA = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 436, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
    }
    ...
    $successCount = 0;
      ...
      foreach($orders as $ord){
        ...
        if(empty($sfvals_PA[$ord[0]["principal_store_uid"]]['value'])) {  //has no special field and/or blank...
          ...
          if($type == 'i'){
            $rowArr[] = '"IN' . str_pad(substr($ord[0]['invoice_number'],-6), 7, 0, STR_PAD_LEFT) . '"';
          } else {
          ...
          if($type == 'i'){
            ...
            $rowArr[] = '"IN'. str_pad(substr($ord[0]['source_document_number'],-6), 6, 0, STR_PAD_LEFT) . '"';
          ...
          foreach($ord as $d){ //detail rows.
            ...
            if(abs($d['document_qty'])>0){
              ...
              if($type == 'i'){
                ...
                $detArr[] = '" "';  //UNIT - CHAR
                $detArr[] = (substr($d['vat_rate'],0,2)=='15')?21:2;
