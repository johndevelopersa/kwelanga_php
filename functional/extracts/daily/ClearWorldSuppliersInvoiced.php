...
class ClearWorldSuppliersInvoiced {
...
class ClearWorldSuppliersInvoicedInit extends extractController {

  private $principalUid = 216; //uid of principal extract.
  private $filename = 'PAS216[@FSEQ].txt';  //main controller will build full filename with seq. for us.
  private $crnFilename = 'CRN216[@FSEQ].txt';  //credit note filename
  ...
  public function generateOutput(){
    ...
    if (sizeof($psms)>0) {
      $sfvals_PA = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 253, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
      $sfvals_SA = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 257, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
    }
    ...
    $depotTrans = array(
      ...
      185 =>   'BRA', // Brand WH
      187 =>   'BEL', // BELVIGLO
      193 =>   '004', // CLEAR WORLD EASTERN CAPE      
      195 =>   'LOG', // Loginet 
      276 =>   '002', // The Beverage Emporium KZN
      432 =>   'GPD', // GP Distribution
      382 =>   'TJB', // Takealot ClearWorld Gauteng 
      383 =>   'TCT', // Takealot ClearWorld Cape 
      426 =>   'SOK', // Solento - KZN
      393 =>   '008', // Nellwyn Bloem
      390 =>   'EGG', // WE ARE EGG CAPE TOWN
      );
    ...
    $successCount = 0;
    $proceedWithExtract = "Y";

      foreach($orders as $ord){

        if($ord[0]['depot_uid'] != 106) {
        /*-------------------------------------------------*/
        /*            START BUILDING OUTPUT
        /*-------------------------------------------------*/
        
        	$regionArr = (isset($depotTrans[$ord[0]['depot_uid']])) ? $depotTrans[$ord[0]['depot_uid']] : false;
          if((empty($sfvals_PA[$ord[0]["principal_store_uid"]]['value'])) || (empty($sfvals_SA[$ord[0]["principal_store_uid"]]['value']))) {  //has no special field and/or blank...
            
            if(strtoupper($ord[0]["captured_by"]) == "SHOPIFY") {
            	
            	   if($ord[0]['depot_uid'] == 432) {
            	   	
            	   	    $updSp = $this->extractDAO->getManageSpecialField('257', $ord[0]["principal_store_uid"], 'CWJHB');
            	   	    $updSp = $this->extractDAO->getManageSpecialField('254', $ord[0]["principal_store_uid"], 'CWJHB');
            	   	    $updSp = $this->extractDAO->getManageSpecialField('253', $ord[0]["principal_store_uid"], 'ITDELI');
              
                	    $storeAcc      = 'ITDELI';
                      $salesAnalysis = 'CWJHB';
                      $proceedWithExtract = "Y";            	   	
            	   	
            	   } elseif($ord[0]['depot_uid'] == 276) {

            	   	    $updSp = $this->extractDAO->getManageSpecialField('257', $ord[0]["principal_store_uid"], 'CW-KZ');
            	   	    $updSp = $this->extractDAO->getManageSpecialField('254', $ord[0]["principal_store_uid"], 'CW-KZ');
            	   	    $updSp = $this->extractDAO->getManageSpecialField('253', $ord[0]["principal_store_uid"], 'ITDELI');
                      ...
                      $proceedWithExtract = "Y";
            	   	    ...
            	   	    $updSp = $this->extractDAO->getManageSpecialField('257', $ord[0]["principal_store_uid"], 'CW');
            	   	    $updSp = $this->extractDAO->getManageSpecialField('254', $ord[0]["principal_store_uid"], 'CW');
            	   	    $updSp = $this->extractDAO->getManageSpecialField('253', $ord[0]["principal_store_uid"], 'ITDELI');
            ...
            } elseif (strtoupper($ord[0]["captured_by"]) == "SHOPIFY-API") {	
            	
                 if($ord[0]['depot_uid'] == 432) {

            	   	    $updSp = $this->extractDAO->getManageSpecialField('257', $ord[0]["principal_store_uid"], 'CWJHB');
            	   	    $updSp = $this->extractDAO->getManageSpecialField('254', $ord[0]["principal_store_uid"], 'CWJHB');
            	   	    $updSp = $this->extractDAO->getManageSpecialField('253', $ord[0]["principal_store_uid"], 'THETEQ');
                 ...
                 $proceedWithExtract = "N";    	
            }
          } else {
               $storeAcc = trim($sfvals_PA[$ord[0]["principal_store_uid"]]['value']);
               $salesAnalysis = trim($sfvals_SA[$ord[0]["principal_store_uid"]]['value']);
               $proceedWithExtract = "Y";
          }
          if($proceedWithExtract == "Y") {
               $successCount++;
               ...
               // check for invoice discount
               $hDisc = '0';
               
               if ($ord[0]['off_invoice_discount'] <> '') {
                   if(strtoupper(substr($ord[0]['off_invoice_discount_type'],0,1)) == 'P') {
                        $hDisc = $ord[0]['off_invoice_discount'];
                   } else {
                       $hDisc = '0';	
                   }
               } 
               ...
               if($type == 'i'){
                    $rowArr[] = '"INV' . str_pad(substr($ord[0]['document_number'],-5), 6, 0, STR_PAD_LEFT) . '"';
               } else {
                    $rowArr[] = '"CR' . str_pad(substr($ord[0]['alternate_document_number'],-6), 6, 0, STR_PAD_LEFT) . '"';
               }
               ...
               if($type == 'i'){
                 ...
                 $rowArr[] = '"'.trim(str_replace(array('"',"'"),array('',''),$ord[0]["source_document_number"])).'"';
                         $invdate = $this->extractDAO->getSourceDocumentInvoiceDate($this->principalUid, $ord[0]["source_document_number"]);  
                 if ($invdate[0]['invoice_date'] < '2018-04-01') {
                        $oldVATRate = "T";	
                 }         
               }
               $rowArr[] = '"N"';  //IN / EX - CHAR
                 ...
                 if(abs($d['document_qty'])<>0) {
                   ...
                   if($type == 'i'){
                   	 if($d['allow_decimal'] == 'Y') {
                         $detArr[] = abs($d['document_qty']/100);
                   	 } else {
                         $detArr[] = abs($d['document_qty']);
                   	 }
                     if($d['alt_code'] == '3550000') {
                     ...
                     $detArr[] = (substr($d['vat_rate'],0,2)=='15')?14:2;
                     ...
                     if($oldVATRate == "T") {
                       ...
                       $detArr[] = number_format(abs(round(($d['selling_price']* (1+($d['vat_rate/100']))), 2)), 2, '.', '');  //INCLUSIVE PRICE - NUM	
                     }                             
                     $detArr[] = '" "';  //UNIT - CHAR
                     $detArr[] = (substr($d['vat_rate'],0,2)==VAL_VAT_RATE_TBLSTD)?14:2;
                     $detArr[] = '3'; //DISCOUNT TYPE
                     $detArr[] = (abs($d['selling_price'])>0 && abs($d['discount_value'])>0) ?  number_format(round($d['discount_value'] / $d['selling_price'] * 100, 2), 2, '', '') : 0; //DISCOUNT %  //stored value : discount_value backwards calculation.
                   }
                   ...
                   if($d['non_stock_item'] == 'Y') {
                   	...
                   	$pastelType = '6';                	
                   	 ...
                   	 $pastelType = '4';
                      if(substr($d['product_code'],0,8) == "ZEN/AMAR") { 
                      	   	  $newCode  = 'ZEN/AMAR';
                      	   	  $detArr[] = '"' . trim(str_replace(array('"'),array(''),$newCode)) . '"';   
                      } else {
