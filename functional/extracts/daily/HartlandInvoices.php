...
class HartlandInvoicesInit extends extractController {

  private $principalUid = 440; //uid of principal extract.
  private $filename = 'INV440[@FSEQ].csv';  //main controller will build full filename with seq. for us.
  private $crnFilename = 'CRN440[@FSEQ].csv';  //credit note filename
  ...
  public function generateOutput(){
   ...
   //      print_r($orders);
          ...
          // Insert Headings
           ...
           $rowArr[] = 'Principal-CustNo';
           $rowArr[] = 'Cust-No' ;
           $rowArr[] = 'Cust-Name';
           $rowArr[] = 'Invoice Number';
           $rowArr[] = 'Date';
           $rowArr[] = 'Item-No';
           $rowArr[] = 'Item-Description';
           $rowArr[] = 'Qty';
           $rowArr[] = 'Line Total ExclAmount';
           $rowArr[] = 'Line Total VAT';
           $rowArr[] = 'Line Total Incl-Amount';
           $rowArr[] = 'Customer-Order';
           $rowArr[] = 'Territory';
           $rowArr[] = 'Region';
           ...
           foreach($orders as $ord) {
                    ...
                    if(abs($ord[0]['document_qty']) <> 0) {            
                         ...
                         $rowArr[]  = '"'.$filePrefix . substr($ord[0]['invoice_number'],-8).'"';
                         $rowArr[]  = '"'.substr($ord[0]["invoice_date"],8,2) . "/".substr($ord[0]["invoice_date"],5,2) . "/" .substr($ord[0]["invoice_date"],0,4) .'"';
                         $rowArr[]  = trim($ord[0]['product_code']);
                         $rowArr[]  = trim($ord[0]['product_description']);
                         $rowArr[]  = 0 - $ord[0]['document_qty'] ;
                         $rowArr[]  = trim(round($ord[0]['extended_price'],2)) ;
                         $rowArr[]  = trim(round($ord[0]['vat_amount'],2)) ;
                         $rowArr[]  = trim(round($ord[0]['total'],2)) ;
                         $rowArr[]  = trim($ord[0]["customer_order_number"]);
                         $rowArr[]  = trim($ord[0]["Area"]);
                         $rowArr[]  = trim($ord[0]['pnpRegion']) ;
                         $dataArr[] = join(',',$rowArr);
                    }
