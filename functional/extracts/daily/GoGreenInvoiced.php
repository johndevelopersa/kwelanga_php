...
class GoGreenInvoicedInit extends extractController {

  private $principalUid = 441; //uid of principal extract.
  
  private $filename    = 'PAS441[@FSEQ].txt';  //main controller will build full filename with seq. for us.
  private $crnFilename = 'CRN441[@FSEQ].txt';  //credit note filename
  ...
  public function generateOutput(){
    ...
    $recipientUId = $reArr[0]['uid'];
        ...
        if (sizeof($psms)>0) {
          $sfvals_PA = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 601, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
          $sfvals_SA = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 600, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
          $sfvals_PJ = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 602, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");

        }
   
        $depotTrans = array(190   =>   'FPE',
                            149   =>   'FGR',
                            467   =>   'FEL' );

        $preFixArr  = array(190   =>   'DL',
                            149   =>   'TS',
                            467   =>   'CL' );
        ...
        foreach($orders as $ord){
            ...
            /*-------------------------------------------------*/
              ...
              $docPreFix = (isset($preFixArr[$ord[0]['depot_uid']])) ? $preFixArr[$ord[0]['depot_uid']] : false;
              ...
              if((empty($sfvals_PA[$ord[0]["principal_store_uid"]]['value'])) ) {  //has no special field and/or blank...
                ...
                $projectCode   = trim($sfvals_PJ[$ord[0]["principal_store_uid"]]['value']);
                ...
                if($type == 'i'){
                  $rowArr[] = '"' . $docPreFix . str_pad(substr($ord[0]['document_number'],-6), 6, 0, STR_PAD_LEFT) . '"';
                } else {
                  $rowArr[] = '"' . $docPreFix . str_pad(substr($ord[0]['document_number'],-6), 6, 6, STR_PAD_LEFT) . '"';
                }
                ...
                $rowArr[] = '""';
                $rowArr[] = '"'.trim($SalesAnalysis).'"'; //SALES ANALYSIS – CHAR
                $rowArr[] = '""';
                if($type == 'i'){
                  $rowArr[] = '"'.date("d/m/Y", strtotime($ord[0]["invoice_date"])).'"';  //DATE (DD/MM/YYYY)
                } else {
                ...
                foreach($ord as $d){ //detail rows.
                  ...
                  if(abs($d['document_qty'])<>0){
                    ...
                    if($type == 'i'){
                      ...
                      $detArr[] = (substr($d['vat_rate'],0,2)=='15')?01:2;
