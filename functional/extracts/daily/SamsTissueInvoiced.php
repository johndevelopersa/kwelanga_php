...
class SamsTissueInvoicedInit extends extractController {

  private $principalUid = 390; //uid of principal extract.
  private $filename = 'PAS390[@FSEQ].txt';  //main controller will build full filename with seq. for us.
  private $crnFilename = 'CRN390[@FSEQ].txt';  //credit note filename
  ...
  public function generateOutput(){
    ...
    if (sizeof($psms)>0) {
      $sfvals_PA = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 521, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
   }
    ...
    foreach($orders as $ord){
        ...
        if(empty($sfvals_PA[$ord[0]["principal_store_uid"]]['value'])) {  //has no special field and/or blank...
            ...
            if($type == 'i') {
            	   $periodDate = strtotime($ord[0]["delivery_date"]);
            } else {
            	   $periodDate = strtotime($ord[0]["invoice_date"]);
            }
            
            
            $period = '00';
            switch (date("m", $periodDate)) {
              case '01':
                $period = '13';
                break;
              case '02':
            ...
            if($type == 'i'){
              $rowArr[] = '"'.date("d/m/Y", strtotime($ord[0]["delivery_date"])).'"';  //DATE (DD/MM/YYYY)
            ...
            if($type == 'i'){
                ...
                if(trim($ord[0]['claim_number']) <> '') {
                  ...
                  $rowArr[] = '"' . ltrim($ord[0]['source_document_number'],'0') . '"';
                }    
            }
            $rowArr[] = '""';   //MESSAGE - CHAR
            $rowArr[] = '""';   //MESSAGE - CHAR
      ...
      if(count($successSEUIdArr)>0){
        ...
        //determine seq.
        $seqFilename = $this->setFilenameFSEQ((($type=='i')?$this->filename:$this->crnFilename), $this->principalUid, false, 4, self::setFilenameFSEQ_LenType_PAD);
