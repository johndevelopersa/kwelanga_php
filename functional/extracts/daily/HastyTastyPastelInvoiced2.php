...
class HastyTastyPastelInvoiced2 {
...
class HastyTastyPastelInvoiced2Init extends extractController {

  private $principalUid = 71; //uid of principal extract.
  private $filename = 'PAS71[@FSEQ].csv';  //main controller will build full filename with seq. for us.
  private $crnFilename = 'CRN71[@FSEQ].csv';  //credit note filename
  ...
  public function generateOutput(){
    ...
    $successCount = 0;
      ...
      foreach($orders as $ord){
        ...
        if(empty($sfvals_PA[$ord[0]["principal_store_uid"]]['value'])) {  //has no special field and/or blank...
          ...
          if($type == 'i'){
            ...
            $rowArr[] = '"'.trim(str_replace(array('"',"'"),array('',''), "IA" . $invdate[0]["invoice_number"])).'"';
            // Get VAT Rate of the original invoice
      ...
      if (sizeof($errorSEUIdArr) > 0) {
        $bIResult = $this->postBIDAO->setSmartEventStatusBulk(implode(",", $errorSEUIdArr), "355", "", FLAG_ERRORTO_ERROR);
        if ($bIResult->type != FLAG_ERRORTO_SUCCESS) {
          $this->errorTO->type = FLAG_ERRORTO_ERROR;
