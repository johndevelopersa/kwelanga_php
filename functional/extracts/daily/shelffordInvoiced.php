...
class shelffordInvoicedInit extends extractController {

  private $principalUid = 445; //uid of principal extract.
  
  private $filename    = 'PAS445[@FSEQ].txt';  //main controller will build full filename with seq. for us.
  private $crnFilename = 'CRN445[@FSEQ].txt';  //credit note filename
  ...
  public function generateOutput(){
    ...
    $recipientUId = $reArr[0]['uid'];
        ...
        if (sizeof($psms)>0) {
          $sfvals_PA = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 616, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
          $sfvals_SA = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 617, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");

        }
   
        $depotTrans = array(7  =>   '025');
            ...
            /*-------------------------------------------------*/
              ...
              if((empty($sfvals_PA[$ord[0]["principal_store_uid"]]['value'])) ) {  //has no special field and/or blank...
                ...
                if($type == 'i'){
                  ...
                  $rowArr[] = '"' . $docPreFix . str_pad(substr($ord[0]['alternate_document_number'],-6), 6, 6, STR_PAD_LEFT) . '"';
                }
