...
class StepInTimeMarketingInvoiced {
...
class StepInTimeMarketingInit extends extractController {
...
echo "Over Here";
      ...
      $typArr = array(DT_CREDITNOTE, DT_MCREDIT_OTHER, DT_MCREDIT_PRICING, DT_MDEBIT_NOTE, 'PP')
      //credits and debit notes
      $rTO = $this->postExtractDAO->queueAllCreditsAndDebits($this->principalUid, $recipientUId, $typArr);  //use the loaded receipientUID and not the notification type... *** same as document confirmations***
      if ($rTO->type != FLAG_ERRORTO_SUCCESS) {
    ...
    foreach($seDocs as $k=>$r){
          ...
          $type = 'i';
          if (in_array($r['document_type_uid'], $typArr){
                $type = 'c';
          }
    ...
    if (sizeof($psms)>0) {
      $sfvals_PA = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 342, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
    ...
    foreach($grpDocs as $type => $orders){
        ...
        foreach($orders as $ord){
           ...
           if(empty($sfvals_PA[$ord[0]["principal_store_uid"]]['value'])) {  //has no special field and/or blank...
              ...
              if($type == 'i'){
                 $rowArr[] = '"E' . ltrim($ord[0]['document_number'],"0") . '"';
              } else {
