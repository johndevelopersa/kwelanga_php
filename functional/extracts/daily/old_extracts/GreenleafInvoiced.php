...
class GreenleafInvoicedInit extends extractController {

  private $principalUid = 208; //uid of principal extract.
  private $filename = 'PAS208[@FSEQ].csv';  //main controller will build full filename with seq. for us.
  private $crnFilename = 'CRN208[@FSEQ].csv';  //credit note filename
  ...
  public function generateOutput(){
    ...
    if (sizeof($psms)>0) {
      $sfvals_PA = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 232, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
      $sfvals_SA = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 290, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
    }
    ...
    $successCount = 0;
      ...
      foreach($orders as $ord){
        ...
        if(
            (empty($sfvals_PA[$ord[0]["principal_store_uid"]]['value'])) || (empty($sfvals_SA[$ord[0]["principal_store_uid"]]['value']))) 
           {  //has no special field and/or blank...
          ...
          if($type == 'i'){
              if($region == '020'){
                  $rowArr[] = '"LB' . str_pad(substr($ord[0]['document_number'],-6), 6, 0, STR_PAD_LEFT) . '"';
              }else {
                  $rowArr[] = '"UL' . str_pad(substr($ord[0]['document_number'],-6), 6, 0, STR_PAD_LEFT) . '"';
              }
          } else {
              if($region == '020'){
                  $rowArr[] = '"CB' . str_pad(substr($ord[0]['alternate_document_number'],-6), 6, 0, STR_PAD_LEFT) . '"';
              }else {
          ...
          if($type == 'i'){
            ...
            $rowArr[] = '"'.trim(str_replace(array('"',"'"),array('',''),$ord[0]["reason_description"])).'"';
          }
          $rowArr[] = '""';   //MESSAGE - CHAR
      ...
      if (sizeof($errorSEUIdArr) > 0) {
        $bIResult = $this->postBIDAO->setSmartEventStatusBulk(implode(",", $errorSEUIdArr), "232,290", "", FLAG_ERRORTO_ERROR);
        if ($bIResult->type != FLAG_ERRORTO_SUCCESS) {
          $this->errorTO->type = FLAG_ERRORTO_ERROR;
