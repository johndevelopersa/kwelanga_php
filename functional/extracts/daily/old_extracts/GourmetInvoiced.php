...
class GourmetInvoicedInit extends extractController {

  private $principalUid = 160; //uid of principal extract.
  private $filename = 'PAS60[@FSEQ].csv';  //main controller will build full filename with seq. for us.
  private $crnFilename = 'CRN60[@FSEQ].csv';  //credit note filename
  ...
  public function generateOutput(){
    ...
    if (sizeof($psms)>0) {
      $sfvals = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 199, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
      $sfvalsSalesAnalysis = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 206, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
    }
    ...
    $successCount = 0;
      ...
      foreach($orders as $ord){
        ...
        if(empty($sfvals[$ord[0]["principal_store_uid"]]['value']) || empty($sfvalsSalesAnalysis[$ord[0]["principal_store_uid"]]['value'])){  //has no special field and/or blank...
          ...
          $salesAnalysis = trim($sfvalsSalesAnalysis[$ord[0]["principal_store_uid"]]['value']);
          ...
          if($type == 'i'){
            $rowArr[] = '"IS' . str_pad(substr($ord[0]['document_number'],-6), 6, 0, STR_PAD_LEFT) . '"';
          } else {
            $rowArr[] = '"CS' . str_pad(substr($ord[0]['alternate_document_number'],-6), 6, 0, STR_PAD_LEFT) . '"';
          }
      ...
      if (sizeof($errorSEUIdArr) > 0) {
        $bIResult = $this->postBIDAO->setSmartEventStatusBulk(implode(",", $errorSEUIdArr), "199,206", "", FLAG_ERRORTO_ERROR);
        if ($bIResult->type != FLAG_ERRORTO_SUCCESS) {
          $this->errorTO->type = FLAG_ERRORTO_ERROR;
