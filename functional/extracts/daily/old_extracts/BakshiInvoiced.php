...
class BakshiInvoicedInit extends extractController {


  private $principalUid = 165; //uid of principal extract.
  private $filename = 'PAS65[@FSEQ].csv';  //main controller will build full filename with seq. for us.
  private $creditFilename = 'CRN65[@FSEQ].csv';  //main controller will build full filename with seq. for us.
  ...
  public function generateOutput(){
    ...
    if (sizeof($psms)>0) {
      $sfvalsACC1 = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 195, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
      $sfvalsACC2 = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 204, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
    }
    ...
    foreach($grpDocs as $type => $docArr){
        ...
        $successCount = 0;
        $filename = ($type == 'c') ? $this->creditFilename : $this->filename;
        ...
        // if the product is PRIVATE use the Account field => 2,
        // which can either be: THE PNPPJ ACCOUNT or the STORE/PASTEL ACC FOR INDEPENDANTS.
        $sfvals = (trim($ord[0]['major_category']) == '176') ? $sfvalsACC2 : $sfvalsACC1;
        ...
        if(empty($sfvals[$ord[0]["principal_store_uid"]]['value'])){  //has no special field and/or blank...
          ...
          if($type == 'c'){
            $rowArr[] = '"CL' . str_pad(substr($ord[0]['alternate_document_number'],-6), 6, 0, STR_PAD_LEFT) . '"';
          } else {
            $rowArr[] = '"IL' . str_pad(substr($ord[0]['document_number'],-6), 6, 0, STR_PAD_LEFT) . '"';
          }
          ...
          if($type == 'c'){
            $rowArr[] = '"IL' . str_pad(substr($ord[0]['source_document_number'],-6), 6, 0, STR_PAD_LEFT) . '"';
          } else {
      ...
      $postingDistributionTO->subject = ($type == 'c') ? $this->getTemplateCreditSubject() : $this->getTemplateInvoiceSubject(); //might have no filename if all errors therefore don't display on subject line...
      ...
      if (sizeof($errorSEUIdArr) > 0) {
        $bIResult = $this->postBIDAO->setSmartEventStatusBulk(implode(",", $errorSEUIdArr), "195,204", "", FLAG_ERRORTO_ERROR);
        if ($bIResult->type != FLAG_ERRORTO_SUCCESS) {
          $this->errorTO->type = FLAG_ERRORTO_ERROR;
