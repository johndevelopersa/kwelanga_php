...
class MorgensterInvoicedInit extends extractController {


  private $principalUid = 15; //uid of principal extract.
  private $filename = 'PAS33[@FSEQ].csv';  //main controller will build full filename with seq. for us.
  ...
  public function generateOutput(){
    ...
    foreach($seDocs as $k=>$r){
      ...
      $pcms[$r["principal_chain_uid"]]=$r["principal_chain_uid"];
    }

    // get special field values for all stores in above docs
    if (sizeof($psms)>0) {
      $sfvals = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 14, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
      $sfSalesCode = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 197, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
    }

    if (sizeof($pcms)>0) {
      $sfProduct = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 198, implode(",",$pcms), CT_CHAIN_SHORTCODE, $arrayIndex="entity_uid");
    }


    foreach($grpDocs as $depot => $docs){
      ...
      foreach($docs as $ord){
        ...
        if(empty($sfvals[$ord[0]["principal_store_uid"]]['value'])){  //has no special field and/or blank...
          ...
          $scde = !empty($sfSalesCode[$ord[0]["principal_store_uid"]]['value']) ? $sfSalesCode[$ord[0]["principal_store_uid"]]['value'] : ' ';
          $docNo = !empty($ord[0]['invoice_number']) ? $ord[0]['invoice_number'] : $ord[0]['document_number'];
          $productPrepend = !empty($sfProduct[$ord[0]["principal_chain_uid"]]['value']) ? $sfProduct[$ord[0]["principal_chain_uid"]]['value'] : '';
          ...
          $rowArr = array();
          $rowArr[] = '"HEADER"';
          $rowArr[] = '"I' . str_pad(substr($docNo,-6), 6, 0, STR_PAD_LEFT) . '"';
          $rowArr[] = '" "';  //space(1);
          $rowArr[] = '"Y"';  //printed
          ...
          $rowArr[] = '"N"';  //IN / EX - CHAR
          $rowArr[] = '"0"';  //discount
          $rowArr[] = '"' . str_pad(substr($ord[0]['document_number'],-6), 6, 0, STR_PAD_LEFT) . '"';  //MESSAGE - CHAR
          $rowArr[] = '""';   //MESSAGE - CHAR
          $rowArr[] = '""';   //MESSAGE - CHAR
          ...
          $rowArr[] = '""'; //del 4.
          $rowArr[] = '"'.$scde.'"'; //SALES ANALYSIS - CHAR
          $rowArr[] = '""';
          ...
          foreach($ord as $d){ //detail rows.
           ...
           # if(abs($d['document_qty'])>0){ //include zero qty.
              ...
              $detArr[] = '4';  //unknown.
              $detArr[] = '" "';              
              if($d['depot_uid']==2){$detArr[]='006';} else {$detArr[]='005' ;}  //unknown.
      ...
      if (sizeof($errorSEUIdArr) > 0) {
        $bIResult = $this->postBIDAO->setSmartEventStatusBulk(implode(",", $errorSEUIdArr), "14", "", FLAG_ERRORTO_ERROR);
        if ($bIResult->type != FLAG_ERRORTO_SUCCESS) {
          $this->errorTO->type = FLAG_ERRORTO_ERROR;
