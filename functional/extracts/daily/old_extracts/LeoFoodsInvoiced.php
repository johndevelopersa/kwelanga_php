...
class LeoFoodsInvoicedInit extends extractController {


  private $principalUid = 117; //uid of principal extract.
  private $filename = 'PAS29[@FSEQ].csv';  //main controller will build full filename with seq. for us.
  ...
  public function generateOutput(){
    ...
    if (sizeof($psms)>0) {
      // $sfvals = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 150, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
      $sfvals_PnPVABakery = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 219, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
      $sfvals_PnPVAConvenience = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 220, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
      $sfvals_PnPVAGroceries = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 221, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
    }
    ...
    foreach($grpDocs as $ord){
      ...
      // we can assume that same category will be across all orders and that LEO Foods only does business with PnP
      if ($ord[0]["major_category"]=="159") $storeAcc = ((isset($sfvals_PnPVABakery[$ord[0]["principal_store_uid"]]))?trim($sfvals_PnPVABakery[$ord[0]["principal_store_uid"]]['value']):false);
      else if ($ord[0]["major_category"]=="160") $storeAcc = ((isset($sfvals_PnPVAConvenience[$ord[0]["principal_store_uid"]]))?trim($sfvals_PnPVAConvenience[$ord[0]["principal_store_uid"]]['value']):false);
      else if ($ord[0]["major_category"]=="161") $storeAcc = ((isset($sfvals_PnPVAGroceries[$ord[0]["principal_store_uid"]]))?trim($sfvals_PnPVAGroceries[$ord[0]["principal_store_uid"]]['value']):false);
      else $storeAcc = false; // ((isset($sfvals[$ord[0]["principal_store_uid"]]))?trim($sfvals[$ord[0]["principal_store_uid"]]['value']):false);

      /*-------------------------------------------------*/
      /*            START BUILDING OUTPUT
      /*-------------------------------------------------*/
      if($ord[0]['depot_uid'] == 108){

        //exclude this depot.
        $successSEUIdArr[] = $ord[0]['se_uid'];

      } else if(empty($storeAcc)) { /*if(empty($sfvals[$ord[0]["principal_store_uid"]]['value'])) { */  //has no special field and/or blank...
        ...
        $rowArr = array();
        $rowArr[] = '"HEADER"';
        $rowArr[] = '"IN' . str_pad(substr($ord[0]['document_number'],-6), 6, 0, STR_PAD_LEFT) . '"';
        $rowArr[] = '" "';  //space(1);
        $rowArr[] = '"Y"';  //printed
    ...
    if (sizeof($errorSEUIdArr) > 0) {
      $bIResult = $this->postBIDAO->setSmartEventStatusBulk(implode(",", $errorSEUIdArr), "150,219,220,221", "", FLAG_ERRORTO_ERROR);
      if ($bIResult->type != FLAG_ERRORTO_SUCCESS) {
        $this->errorTO->type = FLAG_ERRORTO_ERROR;
