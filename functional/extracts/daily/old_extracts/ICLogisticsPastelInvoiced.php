...
class ICLogisticsPastelInvoiced {
...
class ICLogisticsPastelInvoicedInit extends extractController {

  private $principalUid = 294; //uid of principal extract.
  private $filename = 'PAS294[@FSEQ].csv';  //main controller will build full filename with seq. for us.
  private $crnFilename = 'CRN294[@FSEQ].csv';  //credit note filename
  private $supFilename = 'SUP294[@FSEQ].csv';  //credit note filename
  ...
  public function generateOutput(){
    ...
    if (!$this->skipInsert) {
      ...
      $rTO = $this->postExtractDAO->queueAllInvoiced($this->principalUid, $recipientUId, $inclCancelled = false, array(DT_SUPPLIER_INVOICE), array(DST_PROCESSED));  //use the loaded receipientUID and not the notification type... *** same as document confirmations***
      if ($rTO->type != FLAG_ERRORTO_SUCCESS) {
        BroadcastingUtils::sendAlertEmail("System Error", "Export failed to call postExtractDAO->queueAllInvoiced in ".get_class($this)." " . $rTO->description, "Y");
      } else {
        $this->dbConn->dbinsQuery("commit;");
 ...
 //   print_r($seDocs);
    ...
    foreach($seDocs as $k=>$r){
      ...
      $type = 'i';
      $salesAnalysis = 'SALES';
      if ($r['document_type_uid'] == DT_SUPPLIER_INVOICE){
        $type = 's';
        $salesAnalysis = '';
    ...
    if (sizeof($psms)>0) {
      $sfvals_PA = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 341, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
    }
      ...
      foreach($orders as $ord){
        ...
        if(empty($sfvals_PA[$ord[0]["principal_store_uid"]]['value'])) {  //has no special field and/or blank...
          ...
          if($type == 'i'){
            ...
            $rowArr[] = '"SV' . str_pad(substr($ord[0]['document_number'],-6), 6, 0, STR_PAD_LEFT) . '"';
          } else {
          ...
          if($type == 'i' || $type == 's'){
            ...
            if($type == 's') {
            	if (trim(str_replace(array('"',"'"),array('',''),$ord[0]["customer_order_number"])) == "") {
            		$corder = "1";
            	} else {
            		$corder = trim(str_replace(array('"',"'"),array('',''),$ord[0]["customer_order_number"]));
            	}
            	  $rowArr[] = '"'.$corder .'"';
            } else {
  ...
  //        	print_r($ord);
          ...
          if($type == 's') {
              ...
              $detArr[] = number_format(abs(round($ord[0]['exclusive_total'], 2)), 2, '.', '');
              $detArr[] = number_format(abs(round($ord[0]['invoice_total'], 2)), 2, '.', '');
              $detArr[] = '" "';  //UNIT - CHAR
              $detArr[] = (substr($d['vat_rate'],0,2)=='14')?1:2;
              $detArr[] = '3'; //DISCOUNT TYPE
              $detArr[] = 0 ;
              $detArr[] = '"2000000"';
              $detArr[] = '"Cost of Sales / Purchases"';
              $detArr[] = '6';  //unknown.
              $detArr[] = '""';
              $detArr[] = '" "'; 
      ...
      if($type == 'i') {
        ...
        $seqFilename = $this->setFilenameFSEQ($this->supFilename, $this->principalUid, false, 3, self::setFilenameFSEQ_LenType_PAD);
      } else {
        ...
        //create actual file to local Ingenious folder.
        $fp = file_put_contents(DIR_DATA_SURESERVER_NON_FTP_FROM_ONEBACK . 'ingenious/' . $seqFilename, $data);
        if($fp != strlen($data)){
          BroadcastingUtils::sendAlertEmail("System Error", "Failed to create extract file!", "Y");
          $this->errorTO->type = FLAG_ERRORTO_ERROR;
      ...
      if($type=='i') {
         ...
         $postingDistributionTO->subject = $this->getTemplateSupplierInvoiceSubject();
      } else {
      	 $this->getTemplateCreditSubject();
      }
      ...
      if (sizeof($errorSEUIdArr) > 0) {
        $bIResult = $this->postBIDAO->setSmartEventStatusBulk(implode(",", $errorSEUIdArr), "341", "", FLAG_ERRORTO_ERROR);
        if ($bIResult->type != FLAG_ERRORTO_SUCCESS) {
          $this->errorTO->type = FLAG_ERRORTO_ERROR;
