...
class RetailTradingBillingInit extends extractController {


  private $principalUid = 171; //uid of principal extract.
  private $filename = 'P[@YEAR][@FSEQ].TXT';  //main controller will build full filename with seq. for us.
  ...
  public function generateOutput(){
    ...
    if (!$this->skipInsert) {
      ...
      $rTO = $this->postExtractDAO->queueAllInvoiced(
                                                      $this->principalUid,
                                                      $recipientUId,
                                                      $inclCancelled = false,
                                                      array(DT_ORDINV, DT_ORDINV_ZERO_PRICE, DT_DELIVERYNOTE),
                                                      array(DST_UNACCEPTED, DST_ACCEPTED, DST_INVOICED, DST_DELIVERED_POD_OK, DST_DIRTY_POD)
                                                    );
    ...
    if (sizeof($psms)>0) {
      $sfvals = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 205, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
    }
    ...
    if(count($successSEUIdArr)>0){
      ...
      //determine seq.
      $seqFilename = $this->setFilenameFSEQ(str_replace('[@YEAR]', date('y'), $this->filename), $this->principalUid, false, 4, self::setFilenameFSEQ_LenType_PAD);
    ...
    $postingDistributionTO->subject = 'RT Billing Invoices : ' . date('F'); //might have no filename if all errors therefore don't display on subject line...
    ...
    if (sizeof($errorSEUIdArr) > 0) {
      $bIResult = $this->postBIDAO->setSmartEventStatusBulk(implode(",", $errorSEUIdArr), "205", "", FLAG_ERRORTO_ERROR);
      if ($bIResult->type != FLAG_ERRORTO_SUCCESS) {
        $this->errorTO->type = FLAG_ERRORTO_ERROR;
