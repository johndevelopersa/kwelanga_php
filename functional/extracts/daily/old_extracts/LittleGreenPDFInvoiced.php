...
class LittleGreenPDFInvoicedInit extends extractController {
  ...
  private $debFilename = 'DEB';  //credit note filename
  ...
  public function generateOutput(){
    ...
    //credits and debit notes
      ...
      $rTO = $this->postExtractDAO->queueAllCreditsAndDebits($this->principalUid, $recipientUId, array(DT_DEBITNOTE));  //use the loaded receipientUID and not the notification type... *** same as document confirmations***
      if ($rTO->type != FLAG_ERRORTO_SUCCESS) {
        BroadcastingUtils::sendAlertEmail("System Error", "Export failed to call postExtractDAO->queueAllCreditsAndDebits in ".get_class($this)." " . $rTO->description, "Y");
      } else {
        $this->dbConn->dbinsQuery("commit;");
    ...
    foreach($grpDocs as $type => $orders){
      ...
      if ($type=='c') {
        $seqFilename = $this->crnFilename.substr($ord[0]["alternate_document_number"],2,6).'.pdf';
      } elseif ($type=='d') {
        $seqFilename = $this->debFilename.substr($ord[0]["document_number"],2,6).'.pdf';
      } else {
        $seqFilename = $this->filename.$ord[0]["client_document_number"].'.pdf';
      }  
      ...
      if ($type=='c') {
            ...
            $postingDistributionTO->subject = $this->getTemplateDebitSubject();
      } else {
