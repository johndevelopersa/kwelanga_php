...
class TOJInvoicedInit extends extractController {


  private $principalUid = 99; //uid of principal extract.
  private $filename = 'I116[@FSEQ].xml';  //invoice filename
  ...
  public function generateOutput(){
    ...
    if (sizeof($psms)>0) {  // get special field values for all stores in above docs
      $sfvals = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, "105", implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
    }
    ...
    foreach($grpDocs as $doc){
      ...
      if(empty($sfvals[$doc[0]["principal_store_uid"]]['value'])){  //has no special field and/or blank...
        ...
        $data .=  "    <Order>\r\n".
                  ...
                  "           <DepartmentID>" . trim($doc[0]['customer_order_number']) . "</DepartmentID>\r\n".
                  ...
                  "           <DeliveryID>" . trim($sfvals[$doc[0]["principal_store_uid"]]['value']) . "</DeliveryID>\r\n".
        ...
        foreach($doc as $i => $d){ //detail rows.
          ...
          $data .="          <Line>\r\n".
                  ...
                  "              <ProductDescription>" . str_replace(array('"',"'",','),array('','',''), strtoupper(trim($doc[$i]['product_description']))) . "</ProductDescription>\r\n".
