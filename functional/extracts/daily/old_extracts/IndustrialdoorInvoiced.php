...
class IndustrialdoorInvoicedInit extends extractController {

  private $principalUid = 266; //uid of principal extract.
  private $filename = 'INV266[@FSEQ].csv';  //main controller will build full filename with seq. for us.
  private $crnFilename = 'SUP266[@FSEQ].csv';  //credit note filename
  ...
  public function generateOutput(){
    ...
    foreach($seDocs as $k=>$r){
      ...
      $type = 'i';
      if ($r['document_type_uid'] == DT_PURCHASE_ORDER){
        $type = 's';
      }
    ...
    if (sizeof($psms)>0) {
      $sfvals_PA = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 317, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
    }
    ...
    $successCount = 0;
      ...
      foreach($orders as $ord){
        ...
        if(empty($sfvals_PA[$ord[0]["principal_store_uid"]]['value'])) {  //has no special field and/or blank...
          ...
          if(trim($ord[0]["customer_order_number"]) <> '') {
      ...
      $postingDistributionTO->subject = (($type=='i')?$this->getTemplateInvoiceSubject():$this->getTemplatePurchaceOrderSubject()); //might have no filename if all errors therefore don't display on subject line...
      ...
      if (sizeof($errorSEUIdArr) > 0) {
        $bIResult = $this->postBIDAO->setSmartEventStatusBulk(implode(",", $errorSEUIdArr), "317", "", FLAG_ERRORTO_ERROR);
        if ($bIResult->type != FLAG_ERRORTO_SUCCESS) {
          $this->errorTO->type = FLAG_ERRORTO_ERROR;
