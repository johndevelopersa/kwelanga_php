...
class ZambeziInvoicedInit extends extractController {


  private $principalUid = 48; //uid of principal extract.
  private $filename = 'PAS64[@FSEQ].TXT';  //main controller will build full filename with seq. for us.
  ...
  public function generateOutput(){
    ...
    if (sizeof($psms)>0) {
      $sfvals = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 67, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
      $sfvalsREPCODE = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 200, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
    }
    ...
    foreach($grpDocs as $ord){
      ...
      //if region not caterd for, ie: non rt depot, don't send them these orders.
      if($regionArr===false){
        ...
        if(empty($sfvals[$ord[0]["principal_store_uid"]]['value'])){  //has no special field and/or blank...
          ...
          $scdeCode = (!empty($sfvalsREPCODE[$ord[0]["principal_store_uid"]]['value'])) ? trim($sfvalsREPCODE[$ord[0]["principal_store_uid"]]['value']) : '';
          ...
          $rowArr = array();
          $rowArr[] = '"HEADER"';
          $rowArr[] = '"'.str_pad(substr($ord[0]['document_number'],-6), 8, 0, STR_PAD_LEFT).'"';
          $rowArr[] = '" "';  //space(1);
          $rowArr[] = '"Y"';  //printed
          ...
          $rowArr[] = '"    "';
          $rowArr[] = '"'.$scdeCode.'"'; //SALES ANALYSIS - CHAR
          $rowArr[] = '""';
    ...
    if (sizeof($errorSEUIdArr) > 0) {
      $bIResult = $this->postBIDAO->setSmartEventStatusBulk(implode(",", $errorSEUIdArr), "67", "", FLAG_ERRORTO_ERROR);
      if ($bIResult->type != FLAG_ERRORTO_SUCCESS) {
        $this->errorTO->type = FLAG_ERRORTO_ERROR;
