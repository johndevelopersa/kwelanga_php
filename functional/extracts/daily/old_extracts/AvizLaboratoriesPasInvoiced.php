...
class AvizLaboratoriesPasInvoiced {
...
class AvizLaboratoriesPasInvoicedInit extends extractController {

  private $principalUid = 188; //uid of principal extract.
  private $filename = 'PAS188[@FSEQ].csv';  //main controller will build full filename with seq. for us.
  private $crnFilename = 'CRN188[@FSEQ].csv';  //credit note filename
  ...
  public function generateOutput(){
    ...
    if (sizeof($psms)>0) {
      $sfvals_PA = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 292, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
