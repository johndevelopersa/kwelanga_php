...
class CarmienInvoicedInit extends extractController {


  private $principalUid = 32; //uid of principal extract.
  private $filename = 'PAS32[@FSEQ].csv';  //main controller will build full filename with seq. for us.
  private $crnFilename = 'CRN32[@FSEQ].csv';  //credit note filename
  ...
  public function generateOutput(){
    ...
    if (sizeof($psms)>0) {
      $sfvals = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 18, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
    }
     ...
     foreach($grpDocs as $type => $orders){
       ...
       foreach($orders as $ord){
        ...
        if($ord[0]['depot_uid'] == 108){
        ...
        } else if(empty($sfvals[$ord[0]["principal_store_uid"]]['value'])){  //has no special field and/or blank...
          ...
          if($type == 'i'){
            ...
            $rowArr[] = '"'.trim(str_replace(array('"',"'"),array('',''),$ord[0]["customer_order_number"])). ' - IN' . str_pad(substr($ord[0]['document_number'],-6), 6, 0, STR_PAD_LEFT).'"';
          } else {
          ...
          foreach($ord as $d){ //detail rows.
            ...
            if(abs($d['document_qty'])>0){
              ...
              $detArr[] = '4';  //unknown.
              $detArr[] = '""';
              if($ord[0]['depot_uid'] == 2){
                $detArr[] = '"GAU"';
              } else if($ord[0]['depot_uid'] == 3){
                $detArr[] = '"KZN"';
              } else {
                $detArr[] = '" "';
              }
    ...
    if (sizeof($errorSEUIdArr) > 0) {
      $bIResult = $this->postBIDAO->setSmartEventStatusBulk(implode(",", $errorSEUIdArr), "18", "", FLAG_ERRORTO_ERROR);
      if ($bIResult->type != FLAG_ERRORTO_SUCCESS) {
        $this->errorTO->type = FLAG_ERRORTO_ERROR;
