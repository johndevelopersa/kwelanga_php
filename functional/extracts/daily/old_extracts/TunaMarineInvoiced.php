...
class TunaMarineInvoicedInit extends extractController {


  private $principalUid = 78; //uid of principal extract.
  private $invFilename = 'INV21[@FSEQ].TXT';  //invoice filename
  private $crnFilename = 'CRN21[@FSEQ].TXT';  //credit & debit note filename
  ...
  public function generateOutput(){
    ...
    foreach($grpDocs as $type => $docArr){
      ...
      foreach($docArr as $doc){
        ...
        if($type == 'c'){
          $data .= str_pad(substr(abs($doc[0]['invoice_number']),-6), 6, '0', STR_PAD_LEFT) .
        ...
        foreach($doc as $i => $d){ //detail rows.
          ...
          $data .= 'ODTL121' .
                   $docNo .
                   str_pad($pageNo, 2, ' ', STR_PAD_LEFT) .
                   str_pad($lineNo, 2, ' ', STR_PAD_LEFT) .
      ...
      if(count($successSEUIdArr)>0){
        ...
        //determine seq.
        $seqFilename = $this->setFilenameFSEQ($filename, $this->principalUid, $fSeq, 3);
        ...
        $fp = file_put_contents(DIR_DATA_SURESERVER_NON_FTP_FROM . 'tuna/invoices/' . $seqFilename, $data);
        if($fp != strlen($data)){
          BroadcastingUtils::sendAlertEmail("System Error", "Failed to create extract file!", "Y");
          $this->errorTO->type = FLAG_ERRORTO_ERROR;
