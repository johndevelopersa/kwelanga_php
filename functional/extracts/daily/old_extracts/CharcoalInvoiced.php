...
class CharcoalInvoicedInit extends extractController {


  private $principalUid = 93; //uid of principal extract.
  private $filename = 'INV50[@FSEQ].TXT';  //main controller will build full filename with seq. for us.
  ...
  public function generateOutput(){
    ...
    $folder = $this->principalUid . '_charcoal'; // . explode(' ',  strtolower($pArr[0]['principal_name']))[0]; //folder replaced with principal id + first WORD of principal.
    ...
    if (sizeof($psms)>0) {
      $sfvalsSTORE = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 201, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
      $sfvalsBRANCH = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 202, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
    }
    ...
    foreach($grpDocs as $ord){
      ...
      /*-------------------------------------------------*/
      if(empty($sfvalsSTORE[$ord[0]["principal_store_uid"]]['value']) || empty($sfvalsBRANCH[$ord[0]["principal_store_uid"]]['value'])){  //has no special field and/or blank...
        ...
        $storeAcc = trim($sfvalsSTORE[$ord[0]["principal_store_uid"]]['value']);
        $branchAcc = trim($sfvalsBRANCH[$ord[0]["principal_store_uid"]]['value']);
        ...
        $rowArr = array();
        $rowArr[] = '"HEADER"';
        $rowArr[] = '"' . str_pad(substr($ord[0]['invoice_number'],-6), 6, 0, STR_PAD_LEFT) . '"';
        $rowArr[] = '" "';  //space(1);
        $rowArr[] = '"Y"';  //printed
        ...
        $rowArr[] = '"'.$branchAcc.'"';  //CUSTOMER CODE - Pastel Account.
