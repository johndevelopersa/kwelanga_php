...
class OsmansInvoicedInit extends extractController {

  private $principalUid = 261; //uid of principal extract.
  private $filename = 'PAS261[@FSEQ].csv';  //main controller will build full filename with seq. for us.
  private $crnFilename = 'CRN261[@FSEQ].csv';  //credit note filename
  ...
  public function generateOutput(){
    ...
    if (sizeof($psms)>0) {
      $sfvals_PA = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 316, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
    }
