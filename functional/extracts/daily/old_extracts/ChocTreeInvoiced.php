...
class ChocTreeInvoicedInit extends extractController {

  private $principalUid = 33; //uid of principal extract.
  private $filename = 'PAS33[@FSEQ].txt';  //main controller will build full filename with seq. for us.
  private $crnFilename = 'CRN33[@FSEQ].txt';  //credit note filename
  ...
  public function generateOutput(){
    ...
    if (sizeof($psms)>0) {
      $sfvals_PA = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 256, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
    }
...
//$Ffp = rename(DIR_DATA_SURESERVER_NON_FTP_FROM . 'data/' . $seqFilename, DIR_PHPBACKEND_PASTEL_DATAFILES . $seqFilename);
      ...
      if (sizeof($errorSEUIdArr) > 0) {
        $bIResult = $this->postBIDAO->setSmartEventStatusBulk(implode(",", $errorSEUIdArr), "256", "", FLAG_ERRORTO_ERROR);
        if ($bIResult->type != FLAG_ERRORTO_SUCCESS) {
          $this->errorTO->type = FLAG_ERRORTO_ERROR;
