...
class InnuScienceInvoicedInit extends extractController {

  private $principalUid = 121; //uid of principal extract.
  private $filename = 'PAS121[@FSEQ].csv';  //main controller will build full filename with seq. for us.
  private $crnFilename = 'CRN121[@FSEQ].csv';  //credit note filename
  ...
  public function generateOutput(){
    ...
    if (sizeof($psms)>0) {
      $sfvals_PA = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 267, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
    }
    ...
    $successCount = 0;
      ...
      foreach($orders as $ord){
        ...
        if(
          ...
          if($type == 'i'){
          	if ($region == 400) {
          		$rowArr[] = '"JBU' . str_pad(substr($ord[0]['document_number'],-6), 6, 0, STR_PAD_LEFT) . '"';
          	} elseif ($region == 410) {
          		$rowArr[] = '"DBU' . str_pad(substr($ord[0]['document_number'],-6), 6, 0, STR_PAD_LEFT) . '"';
          	} elseif ($region == 420) {
          		$rowArr[] = '"CTU' . str_pad(substr($ord[0]['document_number'],-6), 6, 0, STR_PAD_LEFT) . '"';
          	} elseif ($region == 430) {
          		$rowArr[] = '"PEU' . str_pad(substr($ord[0]['document_number'],-6), 6, 0, STR_PAD_LEFT) . '"';
          	}	
         } else {
						if ($region == 400) {
          		$rowArr[] = '"JBCR'. str_pad(substr($ord[0]['alternate_document_number'],-6), 6, 0, STR_PAD_LEFT) . '"';
          	} elseif ($region == 410) {
          		$rowArr[] = '"DBCR' . str_pad(substr($ord[0]['document_number'],-6), 6, 0, STR_PAD_LEFT) . '"';
          	} elseif ($region == 420) {
          		$rowArr[] = '"CTCR' . str_pad(substr($ord[0]['document_number'],-6), 6, 0, STR_PAD_LEFT) . '"';
          	} elseif ($region == 430) {
          		$rowArr[] = '"PECR' . str_pad(substr($ord[0]['document_number'],-6), 6, 0, STR_PAD_LEFT) . '"';
          	}
         }
          $rowArr[] = '" "';
          $rowArr[] = '"Y"';  //printed
      ...
      if (sizeof($errorSEUIdArr) > 0) {
        $bIResult = $this->postBIDAO->setSmartEventStatusBulk(implode(",", $errorSEUIdArr), "267", "", FLAG_ERRORTO_ERROR);
        if ($bIResult->type != FLAG_ERRORTO_SUCCESS) {
          $this->errorTO->type = FLAG_ERRORTO_ERROR;
