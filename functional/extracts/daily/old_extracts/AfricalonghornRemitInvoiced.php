...
class AfricalonghornRemitInvoiced {
...
class AfricalonghornRemitInvoicedInit extends extractController {

  private $principalUid = 209; //uid of principal extract.
  private $filename = 'RM209[@FSEQ].csv';  //main controller will build full filename with seq. for us.
  ...
  public function generateOutput(){
    ...
    foreach($remitArr as $rm=>$remit){                  
          ...
          $insertTime = substr($remit["insert_datetime"],0,10);
            //period
            if(strtotime($insertTime) >= strtotime("2015-01-01") and strtotime($insertTime) <= strtotime("2015-01-31")){
          	$period = 11;
            $remitdate = "31/01/2015";
          }
          elseif (strtotime($insertTime) >= strtotime("2014-02-01") and strtotime($insertTime) <= strtotime("2014-02-28")){
          	$period = 12;
            $remitdate = "28/02/2015";
          }	
          elseif (strtotime($insertTime)>= strtotime("2014-03-01") and strtotime($insertTime) <= strtotime("2014-03-31")){
          	$period = 1;
            $remitdate = "31/03/2015";
          }	
          elseif (strtotime($insertTime) >= strtotime("2014-04-01") and strtotime($insertTime) <= strtotime("2014-04-30")){
          	$period = 2;
            $remitdate = "30/04/2015";
          }	
          elseif (strtotime($insertTime)>= strtotime("2014-05-01") and strtotime($insertTime) <= strtotime("2014-05-31")){
          	$period = 3;
            $remitdate = "31/05/2014";
          }	
          elseif (strtotime($insertTime) >= strtotime("2014-06-01") and strtotime($insertTime) <= strtotime("2014-06-30")){
          	$period = 4;
            $remitdate = "30/06/2014";
          }	
          elseif (strtotime($insertTime) >= strtotime("2014-07-01") and strtotime($insertTime) <= strtotime("2014-07-31")){
          	$period = 5;
            $remitdate = "31/07/2014";
          }	
          elseif (strtotime($insertTime) >= strtotime("2014-08-01") and strtotime($insertTime) <= strtotime("2014-08-31")){
          	$period = 6;
            $remitdate = "31/08/2014";
          }	
          elseif (strtotime($insertTime) >= strtotime("2014-09-01") and strtotime($insertTime) <= strtotime("2014-09-30")){
          	$period = 7;
            $remitdate = "30/09/2014";
          }	
          elseif (strtotime($insertTime) >= strtotime("2014-10-01") and strtotime($insertTime) <= strtotime("2014-10-31")){
          	$period = 8;
            $remitdate = "31/10/2014";
          }	
          elseif (strtotime($insertTime) >= strtotime("2014-11-01") and strtotime($insertTime) <= strtotime("2014-11-30")){
          	$period = 9;
            $remitdate = "30/11/2014";
          }	
          elseif (strtotime($insertTime) >= strtotime("2014-11-01") and strtotime($insertTime) <= strtotime("2014-12-31")){
          	$period = 10;
