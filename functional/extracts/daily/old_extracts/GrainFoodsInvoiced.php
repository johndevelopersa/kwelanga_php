...
class GrainFoodsInvoicedInit extends extractController {

  private $principalUid = 223; //uid of principal extract.
  private $filename = 'PAS223[@FSEQ].csv';  //main controller will build full filename with seq. for us.
  private $crnFilename = 'CRN223[@FSEQ].csv';  //credit note filename
  ...
  public function generateOutput(){
    ...
    if (sizeof($psms)>0) {
      $sfvals_PA = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 249, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
      $sfvals_SA = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 250, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
    ...
    $successCount = 0;
      ...
      foreach($orders as $ord){
        ...
        $region = (isset($depotTrans[$ord[0]['depot_uid']])) ? $depotTrans[$ord[0]['depot_uid']] : false;
          ...
          if($type == 'i'){
            $rowArr[] = '"IG' . str_pad(substr($ord[0]['document_number'],-6), 6, 0, STR_PAD_LEFT) . '"';
          } else {
            $rowArr[] = '"CG' . str_pad(substr($ord[0]['alternate_document_number'],-6), 6, 0, STR_PAD_LEFT) . '"';
          }
      ...
      if (sizeof($errorSEUIdArr) > 0) {
        $bIResult = $this->postBIDAO->setSmartEventStatusBulk(implode(",", $errorSEUIdArr), "249", "", FLAG_ERRORTO_ERROR);
        if ($bIResult->type != FLAG_ERRORTO_SUCCESS) {
          $this->errorTO->type = FLAG_ERRORTO_ERROR;
