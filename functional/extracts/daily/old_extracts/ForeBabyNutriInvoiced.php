...
class ForeBabyNutriInvoicedInit extends extractController {


  private $principalUid = 184; //uid of principal extract.
  private $invFilename = 'INV155[@FSEQ]i.XML';  //invoice filename
  private $crnFilename = 'INV155[@FSEQ]c.XML';  //credit & debit note filename

  //special field uids
  private $spfUId1 = 212; //shipto
  private $spfUId2 = 211; //billto

  //SGX SETTINGS
  private $recipient = '6002657000017'; // is the same as vendorNo as SGX have already used the proper GLN for another principal
  private $vendorNo = '6009801812076';
  private $vendorName= 'Forebaby Nutritionals';
  private $vendorAddr1= 'NO 24 5TH AVENUE';
  private $vendorAddr2= 'Marlboro';
  private $vendorAddr3= 'Sandton';
  private $vendorAddr4= '2052';
  private $vendorCompanyRegNo = '2012/135535/07';
  private $vendorVatNo = '4170212114';
  private $prefix = 'FN';
  private $site = 'afnar';
      ...
      foreach($docArr as $doc){
        ...
        if(empty($sfvalsShipTO[$doc[0]["principal_store_uid"]]['value']) || empty($sfvalsBillTO[$doc[0]["principal_store_uid"]]['value'])){  //has no special field and/or blank...
          ...
          $data .="             <customer_pono>" . trim($doc[0]['customer_order_number']) . "</customer_pono>\r\n".
                  "             <order_date>" . date('Y-m-d', strtotime($doc[0]['invoice_date'])) . "</order_date>\r\n".
