...
class SizagystixInvoicedInit extends extractController {


  private $principalUid = 131; //uid of principal extract.
  private $filename = 'INV31[@FSEQ].csv';  //main controller will build full filename with seq. for us.
  ...
  public function generateOutput(){
    ...
    if (sizeof($psms)>0) {
      $sfvals = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 184, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
    }
...
//    );
    ...
    foreach($grpDocs as $ord){
      ...
      if(empty($sfvals[$ord[0]["principal_store_uid"]]['value'])){  //has no special field and/or blank...
        ...
        $invPrepend = 'ORD';
        $invoiceNumber = $invPrepend . str_pad(substr($ord[0]['document_number'],-6), 6, 0, STR_PAD_LEFT);
    ...
    if (sizeof($errorSEUIdArr) > 0) {
      $bIResult = $this->postBIDAO->setSmartEventStatusBulk(implode(",", $errorSEUIdArr), "184", "", FLAG_ERRORTO_ERROR);
      if ($bIResult->type != FLAG_ERRORTO_SUCCESS) {
        $this->errorTO->type = FLAG_ERRORTO_ERROR;
