...
class SixbarLogisticsInvoicedInit extends extractController {

  private $principalUid = 145; //uid of principal extract.
  private $filename = 'PAS145[@FSEQ].txt';  //main controller will build full filename with seq. for us.
  private $crnFilename = 'CRN145[@FSEQ].txt';  //credit note filename
  ...
  public function generateOutput(){
    ...
    if (sizeof($psms)>0) {
      $sfvals_PA = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 262, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
    }
    ...
    $successCount = 0;
      ...
      foreach($orders as $ord){
        ...
        /*-------------------------------------------------*/
          ...
          if($type == 'i'){
            ...
            $rowArr[] = '"CN' . str_pad(substr($ord[0]['alternate_document_number'],-6), 6, 0, STR_PAD_LEFT) . '"';
          }
      ...
      if(count($successSEUIdArr)>0){
        ...
        $fp  = file_put_contents(DIR_DATA_SURESERVER_NON_FTP_FROM . 'data/' . $seqFilename, $data);
//$Ffp = rename(DIR_DATA_SURESERVER_NON_FTP_FROM . 'data/' . $seqFilename, DIR_PHPBACKEND_PASTEL_DATAFILES . $seqFilename);
      ...
      if (sizeof($errorSEUIdArr) > 0) {
        $bIResult = $this->postBIDAO->setSmartEventStatusBulk(implode(",", $errorSEUIdArr), "262", "", FLAG_ERRORTO_ERROR);
        if ($bIResult->type != FLAG_ERRORTO_SUCCESS) {
          $this->errorTO->type = FLAG_ERRORTO_ERROR;
