...
class InourgreenhouseInvoicedInit extends extractController {

  private $principalUid = 217; //uid of principal extract.
  private $filename = 'PAS217[@FSEQ].txt';  //main controller will build full filename with seq. for us.
  private $crnFilename = 'CRN217[@FSEQ].txt';  //credit note filename
  ...
  public function generateOutput(){
    ...
    $successCount = 0;
      ...
      if (sizeof($errorSEUIdArr) > 0) {
        $bIResult = $this->postBIDAO->setSmartEventStatusBulk(implode(",", $errorSEUIdArr), "244", "", FLAG_ERRORTO_ERROR);
        if ($bIResult->type != FLAG_ERRORTO_SUCCESS) {
          $this->errorTO->type = FLAG_ERRORTO_ERROR;
