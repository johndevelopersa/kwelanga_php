...
class WavesideInvoicedInit extends extractController {


  private $principalUid = 81; //uid of principal extract.
  private $invFilename = 'INV49[@FSEQ]i.XML';  //invoice filename
  private $crnFilename = 'INV49[@FSEQ]c.XML';  //credit & debit note filename
  private $folder = 'wavesideold';

  //special field uids
  private $spfUId1 = 152; //shipto
  private $spfUId2 = 153; //billto

  //SGX SETTINGS
  private $recipient = '6002657000017';
  private $vendorNo = '6006677000491';
  private $vendorName= 'WAVESIDE (PROPRIETARY) LIMITED';
  private $vendorAddr1= 'PO BOX 9999';
  private $vendorAddr2= 'JOHANNESBURG';
  private $vendorAddr3= '2000';
  private $vendorAddr4= '';
  private $vendorCompanyRegNo = '';
  private $vendorVatNo = '4800121024';
  private $prefix = 'CS';
  private $site = 'csaar';
  ...
  private $smollanInvFilename = 'i49[@FSEQ].txt';
  private $smollanInvTotalFilename = 'si49[@FSEQ].txt';
  private $smollanCreFilename = 'r49[@FSEQ].txt';
  private $smollanCreTotalFilename = 'sr49[@FSEQ].txt';
  private $smollanCanFilename = 'x49[@FSEQ].txt';
  private $smollanFolder = 'smollan';
  ...
  public function generateOutput(){
    ...
    if(gmdate('N') == 5 || gmdate('N') == 6){  //1 = mon, 7 = sun : friday/saturday.
      echo 'No extracts on Friday and Saturdays - combine for Sunday';
      $this->errorTO->type = FLAG_ERRORTO_SUCCESS;
      $this->errorTO->description = "Out of bounds";
      return $this->errorTO;
    }
    ...
    $grpDocsAll = $psms = array();
    foreach($seDocs as $k=>$r){
      //place into groups
      $index = 'i'; //invoices
      if(in_array($r['document_status_uid'], array(DST_CANCELLED, DST_CANCELLED_NOT_OUR_AREA))){
        $index = 'can'; //cancelled
      } else if(in_array($r['document_type_uid'], array(DT_CREDITNOTE, DT_DEBITNOTE))){
        $index = 'c'; //credits
      }
      $grpDocsAll[$index][$r['dm_uid']][] = $r;  //group by index.
    ...
    $grpDocs = $errorSEUIdArr = array(); //update errors at the end.
    foreach($grpDocsAll as $type => $docArr){
      foreach($docArr as $i => $doc){
        ...
        if(empty($sfvalsShipTO[$doc[0]["principal_store_uid"]]['value']) || empty($sfvalsBillTO[$doc[0]["principal_store_uid"]]['value'])){  //has no special field and/or blank...
          ...
          $grpDocs[$type][$i] = $doc; //rebuild array.
        }
      }
    }
    ...
    $successCancelledSEUIdArr = array();
    foreach($grpDocs as $type => $docArr){

      $fileArr = array(); //some have multiple files.
      $fSeqDocumentType = false;

      if($type == 'can'){ //cancelled

        $fSeqDocumentType = 2;
        $lineCount = 1;
        $data = "FH149" . date('YmdHis') . "      " . "\r\n";  //header.
        foreach($docArr as $doc){
          $lineCount++;
          $data .= "HH" . str_pad(abs($doc[0]['document_number']), 9, '0', STR_PAD_LEFT) . "       " . "C" . "\r\n";
          $successCancelledSEUIdArr[] = $doc[0]['se_uid'];
        }
        $data .= "TR" . str_pad($lineCount, 6, '0', STR_PAD_LEFT);
        $fileArr[] = array($this->smollanCanFilename, $data);
        ...
        foreach($docArr as $doc){
          ...
          $lineCount++;
          $docNo = str_pad(abs($doc[0]['source_document_number']), 6, '0', STR_PAD_LEFT);
          $depotCode = (isset($depotTrans[$doc[0]['depot_uid']])) ? $depotTrans[$doc[0]['depot_uid']] : '  ';

          if($doc[0]['depot_uid']==2){  //UJ
            $creditNo = 4 . str_pad(substr($doc[0]['document_number'],-5), 5, '0', STR_PAD_LEFT);
          } else if($doc[0]['depot_uid']==3) { //UD
            $creditNo = 6 . str_pad(substr($doc[0]['document_number'],-5), 5, '0', STR_PAD_LEFT);
          } else if($doc[0]['depot_uid']==5) { //UC
            $creditNo = 5 . str_pad(substr($doc[0]['document_number'],-5), 5, '0', STR_PAD_LEFT);
          } else if($doc[0]['depot_uid']==14) { //UB
            $creditNo = 9 . str_pad(substr($doc[0]['document_number'],-5), 5, '0', STR_PAD_LEFT);
          } else {
            $creditNo = 0 . str_pad(substr($doc[0]['document_number'],-5), 5, '0', STR_PAD_LEFT);
          }
          ...
          $data .= "OHDR149" .
                   ...
                   (($doc[0]['reason_uid']>0)?(str_pad($doc[0]['reason_uid'], 2, ' ', STR_PAD_LEFT)):('99')) .
                   $creditNo .
                   str_pad(substr($doc[0]['claim_number'],0,13), 13, ' ', STR_PAD_RIGHT) .
                   $depotCode . "\r\n";
          ...
          foreach($doc as $i => $d){ //detail rows.
            ...
            $data .= 'ODTL1' .
                     '  ' .
                     $docNo .
                     str_pad($pageNo, 2, '0', STR_PAD_LEFT) .
                     str_pad($lineNo, 2, '0', STR_PAD_LEFT) .
                     str_pad(trim($d['product_code']), 15, ' ', STR_PAD_RIGHT) .
                     str_pad('', 7, ' ') .
                     str_pad(abs($doc[$i]['document_qty']), 8, '0', STR_PAD_LEFT) .
                     (($d['document_type_uid']==DT_DEBITNOTE)?('+'):('-')) .
                     str_pad('', 7, ' ') .
                     str_pad(number_format(round(abs($d['extended_price']), 2), 2, '', ''), 9, '0', STR_PAD_LEFT) .
                     str_pad(number_format(round(abs($d['vat_amount']), 2), 2, '', ''), 9, '0', STR_PAD_LEFT) .
                     str_pad(number_format(round(abs($d['total']), 2), 2, '', ''), 9, '0', STR_PAD_LEFT) .
                     "\r\n";

            $total += round(abs($d['extended_price']), 2);
            $lineCount++;
        ...
        $data .= "TRL01" . str_pad($lineCount+1, 6, '0', STR_PAD_LEFT);

        $dataTotal = '"Date","File","Value"' . "\r\n";
        $dataTotal .= date('d/m/y') . ',"' . "r49[@FSEQ]" . '",' . number_format(round($total, 2), 2, '.', '');

        $fileArr[] = array($this->smollanCreFilename, $data);
        $fileArr[] = array($this->smollanCreTotalFilename, $dataTotal);


      } else if ($type == 'i'){ //invoices
        ...
        foreach($docArr as $doc){
          ...
          $data .= "OHDR149" .
                   $docNo .
                   "00000" .
                   str_pad($doc[0]['old_account'], 13, ' ', STR_PAD_RIGHT) .
                   str_pad($doc[0]['old_account'], 9, ' ', STR_PAD_RIGHT) .
                   date('Ymd', strtotime($doc[0]['order_date'])) .
                   date('Ymd', strtotime($doc[0]['invoice_date'])) .
                   str_pad('', 16, ' ') .
                   $docNo .
          ...
          foreach($doc as $i => $d){ //detail rows.
            ...
            $data .= 'ODTL1' .
                     ...
                     str_pad($lineNo, 2, '0', STR_PAD_LEFT) .
                     str_pad(trim($d['product_code']), 15, ' ', STR_PAD_RIGHT) .
                     str_pad(abs($doc[$i]['ordered_qty']), 8, '0', STR_PAD_LEFT) .
                     str_pad(abs($doc[$i]['document_qty']), 8, '0', STR_PAD_LEFT) .
                     str_pad('', 8, ' ') .
                     str_pad(number_format(round(abs($d['extended_price']), 2), 2, '', ''), 9, '0', STR_PAD_LEFT) .
                     str_pad(number_format(round(abs($d['vat_amount']), 2), 2, '', ''), 9, '0', STR_PAD_LEFT) .
                     str_pad(abs($doc[$i]['document_qty']), 9, '0', STR_PAD_LEFT) .
                     str_pad(number_format(round(abs($d['total']), 2), 2, '', ''), 9, '0', STR_PAD_LEFT) .
                     str_pad(number_format(round(abs($d['discount_value']), 2), 2, '', ''), 9, '0', STR_PAD_LEFT) .
                     "\r\n";
        ...
        $dataTotal = '"Date","File","Value"' . "\r\n";
        $dataTotal .= date('d/m/y') . ',"' . "i49[@FSEQ]" . '",' . number_format(round($total, 2), 2, '.', '');

        $fileArr[] = array($this->smollanInvFilename, $data);
        $fileArr[] = array($this->smollanInvTotalFilename, $dataTotal);

      }


      //credit smollan files.
      if(count($fileArr)>0){

        //create files for each row - total files have same seq....
        //determine seq.
        $seqFilename = $this->setFilenameFSEQ("NOFILENAME", $this->principalUid, $fSeqDocumentType, 3);
        ...
        foreach($fileArr as $fi){
        	
          $filename = str_replace('[@FSEQ]',  str_pad($this->fileSequence,3,"0",STR_PAD_LEFT), $fi[0]);
          $data = str_replace('[@FSEQ]', str_pad($this->fileSequence,3,"0",STR_PAD_LEFT), $fi[1]);  //replace file key in total file.

          //write physical file - placed in extracts folder
          $filePath = $this->createFile($this->principalUid . '_' . $this->smollanFolder, $filename, $data);  //places file in correct folder
          if($filePath == false){
          ...
          $fp = file_put_contents(DIR_DATA_SURESERVER_NON_FTP_FROM . 'smollan/in/' . $filename, $data);
          if($fp != strlen($data)){
            BroadcastingUtils::sendAlertEmail("System Error", "Failed to create extract file!", "Y");
            $this->errorTO->type = FLAG_ERRORTO_ERROR;
        ...
        //UPDATE ANY SMART EVENTS -> cancelled items
        if (sizeof($successCancelledSEUIdArr) > 0) {
          $bIResult = $this->postBIDAO->setSmartEventStatusBulk(implode(",", $successCancelledSEUIdArr), basename($filePath), "");
    ...
    foreach($grpDocs as $type => $docArr){
      ...
      if($type == 'can')  //sgx don't use cancelled orders
        continue;
      ...
      foreach($docArr as $doc){
          ...
          if($type == 'c'){
            $data .="             <order_date>" . date('Y-m-d', strtotime($doc[0]['order_date'])) . "</order_date>\r\n".
                    "             <required_date>" . date('Y-m-d', strtotime($doc[0]['order_date'])) . "</required_date>\r\n";
          } else {
          ...
          $data .="             <order_site>{$this->site}</order_site>\r\n".
                  "             <status>G</status>\r\n".
                  "             <branch>\r\n".
                  "                 <branch_no>" . trim($sfvalsShipTO[$doc[0]["principal_store_uid"]]['value']) . "</branch_no>\r\n".  //SGX Ship to <<<<<<<<<<<<<<<<<<<<<<<<<<<<
                  "                 <branch_name>" . str_replace(array('"',"'",','),array('','',''), htmlentities(trim($doc[0]['deliver_name']))) . "</branch_name>\r\n".
                  "                 <branch_addr1>" . str_replace(array('"',"'",','),array('','',''), htmlentities(trim($doc[0]['deliver_add1']))) . "</branch_addr1>\r\n".
                  "                 <branch_addr2>" . str_replace(array('"',"'",','),array('','',''), htmlentities(trim($doc[0]['deliver_add2']))) . "</branch_addr2>\r\n".
                  "                 <branch_addr3>" . str_replace(array('"',"'",','),array('','',''), htmlentities(trim($doc[0]['deliver_add3']))) . "</branch_addr3>\r\n".
...
//        } //has spf value.
      ...
      if(count($successSEUIdArr)>0 || $isBlankFile){
        ...
        //determine seq.
        $seqFilename = $this->setFilenameFSEQ($filename, $this->principalUid, false, 3);
        if($seqFilename==false){
          BroadcastingUtils::sendAlertEmail("System Error", "Sequence method setFilenameFSEQ failed in ".get_class($this)."!", "Y");
          return $this->errorTO;
        }
        ...
        //write physical file - placed in extracts folder
        $filePath = $this->createFile($this->principalUid . '_' . $this->folder, $seqFilename, $data);  //places file in correct folder
        if($filePath == false){
          BroadcastingUtils::sendAlertEmail("System Error", "Failed to create extract file!", "Y");
          $this->errorTO->type = FLAG_ERRORTO_ERROR;
          $this->errorTO->description = "Failed in ".get_class($this)." on backup file creation";
        ...
        $fp = file_put_contents(DIR_DATA_SURESERVER_NON_FTP_FROM . 'sgx/tosgx/dns/' . $seqFilename, $data);
        if($fp != strlen($data)){
          BroadcastingUtils::sendAlertEmail("System Error", "Failed to create extract file!", "Y");
          $this->errorTO->type = FLAG_ERRORTO_ERROR;
          $this->errorTO->description = "Failed in ".get_class($this)." on actual file creation";
    ...
    //ERRORS, only for SGX - bulk action and email out...
    if (sizeof($errorSEUIdArr) > 0) {
        ...
        $postingDistributionTO->body = $this->getTemplateBodyError($this->vendorName, count($errorSEUIdArr), $this->getManagementURL($this->principalUid));
