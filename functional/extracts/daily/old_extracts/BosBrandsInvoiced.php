...
class BosBrandsInvoicedInit extends extractController {


  private $principalUid = 116; //uid of principal extract.
  private $invFilename = 'I[@FSEQ].csv';  //invoice filename
  private $crnFilename = 'C[@FSEQ].csv';  //credit & debit note filename
  ...
  public function generateOutput(){
    ...
    if (sizeof($psms)>0) {
      $sfvals = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 139, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
    }
    ...
    foreach($grpDocs as $type => $docArr){
        ...
        if($doc[0]['depot_uid'] != 106) {
          if(empty($sfvals[$doc[0]["principal_store_uid"]]['value'])){  //has no special field and/or blank...
            ...
            foreach($doc as $i => $d){ //detail rows.
              ...
              $rowArr[] = '0';
              if($type == 'i'){
                $rowArr[] = 'I' . abs($doc[0]['document_number']);  //document_number
              } else {
                $rowArr[] = abs($doc[0]['alternate_document_number']);  //alternate document_number
              }
              $rowArr[] = $sfvals[$doc[0]["principal_store_uid"]]['value'];
              $rowArr[] = trim($doc[0]['deliver_name']);
              if($type != 'i'){
                $rowArr[] = 'I'.abs($doc[0]['source_document_number']);
              } else {
                $rowArr[] = '';
              }
      ...
      if (sizeof($errorSEUIdArr) > 0) {
        $bIResult = $this->postBIDAO->setSmartEventStatusBulk(implode(",", $errorSEUIdArr), "139", "", FLAG_ERRORTO_ERROR);
        if ($bIResult->type != FLAG_ERRORTO_SUCCESS) {
          $this->errorTO->type = FLAG_ERRORTO_ERROR;
