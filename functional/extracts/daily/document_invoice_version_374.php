               ...
               <?php 
                ...
                ?>
                  <td rowspan="2" style="text-align:right; color:grey; font-weight:normal; font-size:0.55em;">
                      <?php echo "<img src=".$logo." style=width:300px; height:200px; float:right; >" ?>
                  </td>
           <?php } ?>
         </tr>
         <tr>
...
<?php    
    ...
    // Check for active invoice discount
         include_once($ROOT.$PHPFOLDER."DAO/InvoiceDiscountDAO.php");
         
         $InvoiceDiscountDAO = new InvoiceDiscountDAO($dbConn);
         $iDisc = $InvoiceDiscountDAO->fetchInvoiceDiscounts($mfT[0]['principal_store_uid'], 
                                                             $mfT[0]['invoice_date'],  
                                                             $mfT[0]['principal_uid'], 
                                                             $totNett, 1, NULL);    	
         ...
         // Change this the store discount uid
         
     if(count($iDisc) > 0) {
          
           if($iDisc[0]['discount_type'] == 'P') { 
           	
           	           $dType   = $iDisc[0]['discount_amount'] . "%";
           	           $dAmount = $totNett * $iDisc[0]['discount_amount'] / 100 ;
           	           $dVat    = $totVAT  * $iDisc[0]['discount_amount'] / 100 ; 
           	           $dTot    = $totTot  * $iDisc[0]['discount_amount'] / 100 ;   
           	           ...
           	           if($mfT[0]['invoice_discount_uid'] == '') {
           	                 $InvoiceDiscountDAO = new InvoiceDiscountDAO($dbConn);
                             $iDisc = $InvoiceDiscountDAO->storeInvoiceDiscountUid($iDisc[0]['uid']);
                       }          	         
