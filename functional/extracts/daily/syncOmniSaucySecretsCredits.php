...
$reArr = (new BIDAO($dbConn))->getNotificationRecipients(SaucySecretsConstants::PrincipalID, NT_DAILY_EXTRACT_ALTCUSTOM3);
if (count($reArr) == 0) {
...
$documentTypeArr = [DT_CREDITNOTE];

//credits and debit notes

$rTO = (new PostExtractDAO($dbConn))->queueAllCreditsAndDebits(SaucySecretsConstants::PrincipalID,
                                                               $recipientUId, 
                                                               $documentTypeArr,
                                                               $depotUId=false,
...
$seDocs = (new ExtractDAO($dbConn))->getDailyExtractInvoicedOrdersWithParms(SaucySecretsConstants::PrincipalID, 
                                                                            $recipientUId,
                                                                            '',
                                                                            '',
                                                                            $docType=implode(',',$documentTypeArr));
...
foreach($grpDocs as $ord) {
    ...
    $documentNumber = ltrim($ord[0]['alternate_document_number'],'0');
    $clientDocNumber = ltrim($ord[0]['alternate_document_number'],'0');
	  $customerOrderNumber = $ord[0]['source_document_number'];
    ...
    $invdate = (new ExtractDAO($dbConn))->getSourceDocumentInvoiceDate(SaucySecretsConstants::PrincipalID, $ord[0]["source_document_number"]);  
    if ($invdate['invoice_date'] > '2021-04-01') {
                     continue;	
    }
     ...
     *      CREATE ORDER OBJECT
     *-----------------------------------------------------------*/
     $omniOrder = (new OmniCreditNoteObj)
     ...
     */
     foreach ($ord as $d) {     	
           ...
           *-----------------------------------------------------------*/
           ...
           $creditLineItem = (new OmniCreditNoteLineItemObj)
                ...
                //->setWarrantyPeriod() ;
                $creditLineItem->addTrackingNumbers($tSerial_no = 'NA', 
                                                   $tBatch_no = '1', 
                                                   $tTracking_no_3 = "NA", 
                                                   $tTracking_no_4 = "NA", 
                                                  $tQty = 1);
                ...
                //append to order
                $omniOrder->addOrderLine($creditLineItem); 
    ...
    $response = $omniApi->CreateCreditNote($omniOrder, $documentNumber, $DocUid);
...
//           echo "<br> End of Error <br>";
        ...
        echo "[OMNI] Successfully Created Credit Note:" . $responseOrderNo;
 
        /*-------------------------------------------------------------
         *      GET POSTED ORDER AND VERIFY -- ALL OK!
         *------------------------------------------------------------ */
       
        echo "<h1>Successful Credit Note: {$documentNumber}</h1>";
