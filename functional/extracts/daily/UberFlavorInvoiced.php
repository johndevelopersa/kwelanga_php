...
class UberFlavorInvoicedInit extends extractController {

  private $principalUid = 317; //uid of principal extract.
  private $filename = 'PAS317[@FSEQ].csv';  //main controller will build full filename with seq. for us.
  private $crnFilename = 'CRN317[@FSEQ].csv';  //credit note filename
  ...
  public function generateOutput(){
    ...
    if (sizeof($psms)>0) {
      $sfvals_PA = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 348, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
      $sfvals_SA = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 435, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
    ...
    $successCount = 0;
      ...
      foreach($orders as $ord){
        ...
        /*-------------------------------------------------*/
        $regionArr = (isset($depotTrans[$ord[0]['depot_uid']])) ? $depotTrans[$ord[0]['depot_uid']] : 'JHB';
        ...
        if((empty($sfvals_PA[$ord[0]["principal_store_uid"]]['value'])) || (empty($sfvals_SA[$ord[0]["principal_store_uid"]]['value']))) {  //has no special field and/or blank...
          ...
          if($type == 'i'){
            $rowArr[] = '"IN' . str_pad(substr($ord[0]['invoice_number'],-6), 6, 0, STR_PAD_LEFT) . '"';
          } else {
          ...
          foreach($ord as $d){ //detail rows.
            ...
            if(abs($d['document_qty'])>0){
              ...
              if($type == 'i'){
                ...
                $detArr[] = '" "';  //UNIT - CHAR
                $detArr[] = (substr($d['vat_rate'],0,2)=='15')?1:2;
                $detArr[] = '3'; //DISCOUNT TYPE
                $detArr[] = (abs($d['selling_price'])>0 && abs($d['discount_value'])>0) ?  number_format(round($d['discount_value'] / $d['selling_price'] * 100, 2), 2, '', '') : 0; //DISCOUNT %  //stored value : discount_value backwards calculation.
              }
      ...
      if (sizeof($errorSEUIdArr) > 0) {
        $bIResult = $this->postBIDAO->setSmartEventStatusBulk(implode(",", $errorSEUIdArr), "348", "", FLAG_ERRORTO_ERROR);
        if ($bIResult->type != FLAG_ERRORTO_SUCCESS) {
          $this->errorTO->type = FLAG_ERRORTO_ERROR;
