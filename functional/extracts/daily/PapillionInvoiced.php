...
class PapillionInvoicedInit extends extractController {

  private $principalUid = 420; //uid of principal extract.
  private $filename = 'PAS420[@FSEQ].txt';  //main controller will build full filename with seq. for us.
  private $crnFilename = 'CRN420[@FSEQ].txt';  //credit note filename
  ...
  public function generateOutput(){
    ...
    if (sizeof($psms)>0) {
      $sfvals_PA = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 628, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
...
//    print_r($orders);  
      ...
      foreach($orders as $ord){
          ...
          if((empty($sfvals_PA[$ord[0]["principal_store_uid"]]['value']))) {  //has no special field and/or blank...
            ...
            if($type == 'i'){
              $rowArr[] = '"INV' . str_pad(substr($ord[0]['document_number'],-6), 6, 0, STR_PAD_LEFT) . '"';
            } else {
              $rowArr[] = '"CRN' . str_pad(substr($ord[0]['alternate_document_number'],-6), 6, 0, STR_PAD_LEFT) . '"';
            }
