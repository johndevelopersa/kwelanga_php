...
class SafariInvoicedInit extends extractController {

  private $principalUid = 400; //uid of principal extract.
  private $filename = 'PAS400[@FSEQ].txt';  //main controller will build full filename with seq. for us.
  private $crnFilename = 'CRN400[@FSEQ].txt';  //credit note filename
  ...
  public function generateOutput(){
    ...
    if (sizeof($psms)>0) {
      $sfvals_PA = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 534, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
      $sfvals_SA = $this->miscDAO->getPrincipalSpecialFieldValuesMultEntities($this->principalUid, 535, implode(",",$psms), CT_STORE_SHORTCODE, $arrayIndex="entity_uid");
    }
    ...
    $successCount = 0;    
      ...
      foreach($orders as $ord){
          ...
          if((empty($sfvals_PA[$ord[0]["principal_store_uid"]]['value'])) || (empty($sfvals_SA[$ord[0]["principal_store_uid"]]['value']))) {  //has no special field and/or blank...
            ...
            if($type == 'i'){
              $rowArr[] = '"IA' . str_pad(substr($ord[0]['document_number'],-6), 6, 0, STR_PAD_LEFT) . '"';
            } else {
              $rowArr[] = '"CN' . str_pad(substr($ord[0]['document_number'],-6), 6, 0, STR_PAD_LEFT) . '"';
            }
