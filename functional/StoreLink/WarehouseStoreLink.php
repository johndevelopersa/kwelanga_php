 ...
 //*******************************************************************************************************************************  	
      	if (isset($_POST['SEARCHGLN']))
   	{
   	  ...
   	  if (isset($_POST['SELMOD'])){
   		      ...
   		      $link = "<script>window.open('".$ROOT.$PHPFOLDER."functional/StoreLink/LinkStoreCard.php?PRINCIPALUID=".$principalUID ." & WSTOREUID=".$storeUID."', 'popup', 'width=850,height=600')</script>";
...
//********************************************************************************************************************************
 if (isset($_POST['DELMOD'])){
   	       	...
   	       	$principalUID = $_POST['DELMOD'];
   	       	$warehouseStoreUID = $_POST['STOREUID'];
   	       	$WarehouseStoreLinkDAO = new WarehouseStoreLinkDAO($dbConn);
            $Link = $WarehouseStoreLinkDAO->CheckLink($wareHouseCde,$principalUID,$warehouseStoreUID);
   	       	
   	       	$WarehouseStoreLinkDAO = new WarehouseStoreLinkDAO($dbConn);
            $errorTO = $WarehouseStoreLinkDAO->UpdateLink($Link[0]['uid'],'N'); 
   		  
   		    if ($errorTO->type==FLAG_ERRORTO_SUCCESS) { ?>
                    <script type='text/javascript'>parent.showMsgBoxInfo('Link Deleted')</script>  
...
//**********************************************************************************************************************************   		      
      ...
      if (!isset($_POST['SEARCHBRANCH'])&&!isset($_POST['SEARCHGLN'])&&!isset($_POST['SEARCHNAME'])&&!isset($_POST['SELMOD'])&&!isset($_POST['SUBNAME'])&&!isset($_POST['REFRESH'])&&!isset($_POST['DELMOD'])){
