...
public function LinkStores($wareHouseCde,$principalUID,$pStoreUID,$wstoreID){
	...
	//************************************************************
	public function DeleteLink($principalUID){
		
		$sql = "UPDATE principal_warehouse_store_link pl
            SET pl.`status`= 'N'
            WHERE pl.principal_store_master_uid =". mysqli_real_escape_string($this->dbConn->connection,$principalUID) ."";
	...
	//************************************************************
	public function UpdateLink($plUID){
		
		$sql = "UPDATE principal_warehouse_store_link pl
            SET pl.`status`= 'Y'
            WHERE pl.uid =". mysqli_real_escape_string($this->dbConn->connection,$plUID) ."";
	 echo $sql;
	  
	  $this->errorTO = $this->dbConn->processPosting($sql,"");
            
        if ($this->errorTO->type!=FLAG_ERRORTO_SUCCESS) {            	    
            $this->errorTO->description="Failed To Update Link <br><br> Contact Kwelanga Support".$this->errorTO->description;
