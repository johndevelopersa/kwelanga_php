...
// **************************************************************************************
      if (isset($_POST['firstform'])) {
      	    if (trim($postUser) !='Select User' && trim($postTransaction) != 'Select Transaction Type' ) { 
      	    	
      	    	  if($postTransaction == DT_PAYMENT ) {
                     $TransactionDAO = new TransactionDAO($dbConn);
                     $mfTTX = $TransactionDAO->getUnauthorisedPayments($principalId, $postUser, $postTransaction) ; 
      	    	  } else {
                     $TransactionDAO = new TransactionDAO($dbConn);
                     $mfTTX = $TransactionDAO->getUnauthorisedTransactions($principalId, $postUser, $postTransaction) ; 
      	    	  }
                if (sizeof($mfTTX)>0) { 
                    $class = 'odd';
                ?>   
                        ...
                        <form name='Invoices' method=post target=''>
                           ...
                           <table width="900"; style="border:none">
                                <tr class="<?php echo GUICommonUtils::styleEO($class); ?>">
                                   <td colspan=8; style="font-weight:bold;font-size:2em;text-align:center; font-family: Calibri, Verdana, Ariel, sans-serif;" >Payment / Invoice Matching</td>
                                </tr>
                                <tr class="<?php echo GUICommonUtils::styleEO($class); ?>">
                                     <td width="6%";>&nbsp</td>  
                                     <td width="35%";>&nbsp</td>
                                ...
                                <tr class="<?php echo GUICommonUtils::styleEO($class); ?>">
                                     ...
                                     <td style="border:none; font-weight: bold;   font-size: 14px;">Type</td>
                                     <td colspan=3; style="border:none; font-weight: Normal; font-size: 13px;"><?php echo trim($mfTTX[0]['DocType']) ?></td>
                                ...
                                <?php
                                  foreach($mfTTX as $row) { 
                                     ...
                                     <tr class="<?php echo GUICommonUtils::styleEO($class); ?>" >
                                       ...
                                       <td style="border:1px solid black; border-collapse: collapse; font-weight: normal;" ><?php echo trim($row['Status']) ?></td>
     ...
     $TransactionDAO = new TransactionDAO($dbConn);
     $mfPUL = $TransactionDAO->getPrincipalUsers($principalId);   
    ...
    <center>
       <FORM name='Select Invoice' method=post action=''>
            ...
            <table width="600"; style="border:none" >        	
               <tr class="<?php echo GUICommonUtils::styleEO($class); ?>">
                 <td width="30%"; style="border:none">User</td>
                 <td width="20%"; style="border:none">&nbsp</td>
                 <td width="30%"; style="border:none">Document Type</td>
                  ...
                  <td>
                   ...
                   <td>               	
                    <select name="Transaction" id="Transaction">
                      <option value="Select Transaction Type">Transaction Type</option>
                      <?php foreach($mfTTP as $row) {                     ?>
