...
var debriefDocument = {
    ...
    showManage : function(dmUId, rowIndex, thisField, deliveryNote) {
        ...
        $('#div_MANAGECONTENT').load('<?php echo $ROOT.$PHPFOLDER."functional/transaction/new_view_tracking/manageDocument.php?pAlias=&DOCMASTID=" ?>'+dmUId, debriefDocument.isLoadedManage=false);
    },
    ...
    manageAccept : function(dmUIdlist) {
        ...
        if (((pageGroup==3) || (pageGroup==2 && skipInPickStage=='Y')) && ((status=="I") || (status=="W")) ) { // inpick changing to Invoiced
        ...
        if (status=="A") {
            ...
            submitAction('WAITING_DISPATCH',dmUIdlist,params,'debriefDocument.successCallBack("'+dmUIdlist+'",<?php echo ((CommonUtils::isDepotUser())?"true":"true") ?>); refreshStatus("'+dmUIdlist+'",msgClass);');
