  ...
  if ($skipInPickStage!='Y') {
      if ($principalAliasId == 216) {
  	  ...
  	  if ($waitingDispatch =='Y') { 
              BasicInputElement::getCSS3RadioHorizontal("MD_STATUS",
                                                         strtoupper(GUICommonUtils::translateDocumentStatusType($resultStatus)).",WAITING_DISPATCH,CANCEL",
                                                        "{$resultStatus},W,C",
                                                         $resultStatus,
                                                         $disabled = false,
                                                        "mdStatusChange();");  	  	
...
echo "<script type='text/javascript' defer>
 ...
 function renderAmmend() {
   ...
   global $mfTArr, $mfTDArr, $postDOCMASTID;
 ...
 //      	print_r($mfTDArr);
   ...
   foreach($mfTDArr[0] as $r) {
   	...
   	if ($r["non_stock_item"] == 'Y' || $r["principal_uid"] == 342 || $r["depot_uid"] == 284 || $r["principal_uid"] == 365 || $r["principal_uid"] == 305 || $r["principal_uid"] == 390 || $r["depot_uid"] == 119 ) {
       $amended=$r["ordered_qty"];
    } else {   
       $amended=0;
       if (intval($r["closing"])<0) { 
       	$amended=0;
//       	file_put_contents('file.txt', "111", FILE_APPEND);
