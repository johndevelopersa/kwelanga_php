...
if(isset($_POST['CANFORM'])) { 
      ...
      $errorTO = $TripSheetDAO->saveTripSheetDispatch($depId, $tripNo, 'N' );
      
      if($errorTO->type==FLAG_ERRORTO_SUCCESS) { ?>
      	   <script type='text/javascript'>parent.showMsgBoxInfo('Tripsheet dispatch Cancelled')</script> 
      <?php	
      } else {
            ?>
...
if(isset($_POST['SAVEDISPATCH'])) { 
	 if($_SESSION['ddocs'] == $_POST['NODOC']) {
      ...
      $TripSheetDAO = new TripSheetDAO($dbConn);
      $errorTO = $TripSheetDAO->saveTripSheetDispatch($depId, $tripNo, 'Y', $tripSheetDespatch);
...
if(isset($_POST['INVOICENO'])) { 
    ...
    if(count($chTS) <> 0) {    	
          ...
          <script type='text/javascript' >parent.showMsgBoxError("Document Not Found on this Trip Sheet <BR><BR>")</script> 	
          <?php 
          unset($_POST['INVOICENO']);
//	        unset($displayCapture);	 	    	
    }
...
if(isset($_POST['firstform']) && !isset($displayCapture)) { 
...
// ****************************************************************************************************************************************
if(!isset($_POST['TRANSPORT']) && !isset($displayCapture)) {
     $TripsheetDAO = new TripsheetDAO($dbConn);     
     $mfTS = $TripsheetDAO->getTripSheetTransporter2($depotId,'','');
     
     if(count($mfTS) == 0) {?>     	    
           <script type='text/javascript'>parent.showMsgBoxError('No Transports Set Up<br><br> Contact Kwelanga Support')</script>
     <?php     	
     } ?> 
     <center>
        <form name='Create a Tripsheet' method=post action=''>
           <table width="720"; style="border:none">
              <tr class="<?php echo GUICommonUtils::styleEO($class); ?>">
                   <td class=head1 colspan="6"; style="text-align:left";>Create a Trip Sheet</td>
              </tr>
              <tr class="<?php echo GUICommonUtils::styleEO($class); ?>">
                   <td width="10%"; style="border:none">&nbsp;</td>
                   <td width="20%"; style="border:none">&nbsp;</td>
              ...
              <tr class="<?php echo GUICommonUtils::styleEO($cl); ?>">
                   ...
                   <td class="det1" colspan="3" style="text-align:left;">Select Transporter</td>                
              ...
              <tr class="<?php echo GUICommonUtils::styleEO($class); ?>">
               ...
               <tr class="<?php echo GUICommonUtils::styleEO($class); ?>">
                   <td class=det2 colspan="2"></td>
                   <td class=det2 colspan="3" style="text-align:left";><INPUT TYPE="submit" class="submit" name="firstform" value= "Scan Invoices for Trip Sheet">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp
                                                                         <INPUT TYPE="submit" class="submit" name="canform"   value= "Cancel"></td>
                   <td colspan="1"  style="text-align:center;">&nbsp;</td>
               </tr>
              <tr class="<?php echo GUICommonUtils::styleEO($class); ?>">
