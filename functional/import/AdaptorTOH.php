...
//         	preg_replace('/[\xEF\xBB\xBF]/', '', $lineArr[$x]);
      ...
      foreach ($fileArr as $key=>$line) {
           ...
           if(trim($lineArr[$Barcode]) == '' || $lineArr[$Barcode] == '0') {
      	   	   continue;      	   	
      	   }
           
           
           if ($newStore <> trim($lineArr[$Site])) {
           	 echo "<br>";
           	 echo  $lineArr[$Vendor_Name];
             echo "<br>";
             // Determine Principal code from first line of order    
               ...
               if(strtoupper($lineArr[$Vendor_Name]) == 'MAJOR TECH') {
                   ...
                   $currPrincipal = '404';	                   
               } elseif (strtoupper($lineArr[$Vendor_Name]) == 'XXXXX') {
                   $currPrincipal = 'XXX';	                   
               } else {
                   $eTO->type = FLAG_ERRORTO_ERROR;
                   $eTO->description = "Cannnot Determine the Principal!"; 
...
//      echo count($fileArr);
      ...
      if (in_array(trim(substr($fileArr[0],0,7)), $headerarray))   {
         ...
         for ($x = 0; $x <= 22; $x++) {
                 ...
                 $Quote = $x;
                 $validfile++;	
              } elseif(trim($lineArr[$x])=='RecipientsXX') {
                 $Recipients = $x;
                 $validfile++;	
              } elseif(trim($lineArr[$x])=='SignatureXX') {
                 $Signature = $x;
                 $validfile++;	
              } elseif(trim($lineArr[$x])=='Tax') {
                 $Tax = $x;
    ...
    function adaptorTOH_NELLWYN($content, $onlineFileProcessItem) {
         ...
         if (!isset($storeDAO)) $storeDAO = new StoreDAO($this->dbConn);
            $arrTO=array();
             ...
             $headerarray=array('SALES DOCUMENT', 'SALES DOC.');
...
// Sales doc.	Sold-to pt	Delivery Document	Sold-to party	Material	Material Number	Document Date	Order Quantity
             ...
             if (in_array(strtoupper(trim(substr($fileArr[0],0,14))), $headerarray) || in_array(strtoupper(trim(substr($fileArr[0],0,10))), $headerarray) )   {
                   // validate first line of file
                    ...
                    for ($x = 0; $x <= 15; $x++) {
         	              if(strtoupper(trim($lineArr[$x]))=='DD # ' || strtoupper(trim($lineArr[$x]))=='DELIVERY DOCUMENT' || strtoupper(trim($lineArr[$x]))=='DD' || strtoupper(trim($lineArr[$x]))=='DELIVERY') {
                            $docNo = $x;
                            $validfile++;
                        } elseif(strtoupper(trim($lineArr[$x]))=='DOCUMENT DATE' || strtoupper(trim($lineArr[$x]))=='DOC.DATE') {
                            $oDate = $x;
                            $validfile++;                     
                        } elseif(strtoupper(trim($lineArr[$x]))=='REQ.DLV.DT'   || strtoupper(trim($lineArr[$x]))=='DELIVERY DATE') {
                            $divDate = $x;
                            $validfile++;  
                        } elseif(strtoupper(trim($lineArr[$x]))=='SALES DOCUMENT' || strtoupper(trim($lineArr[$x]))=='SALES DOC.') {
                            $PoNo = $x;
                            $validfile++;
                        } elseif(strtoupper(trim($lineArr[$x]))=='ORDER QUANTITY' || strtoupper(trim($lineArr[$x]))=='CONFIRMED QTY' || strtoupper(trim($lineArr[$x]))=='CONFIRMED QUANTITY') {
                            $cases = $x;
                            $validfile++;
                        } elseif(strtoupper(trim($lineArr[$x]))=='NAME 1' || strtoupper(trim($lineArr[$x]))=='SOLD-TO PARTY') {
                            $Customer_name = $x;
                            $validfile++;
                        } elseif(strtoupper(trim($lineArr[$x]))=='SOLD-TO PT' || strtoupper(trim($lineArr[$x]))=='ACCOUNT' || strtoupper(trim($lineArr[$x]))=='XXSOLD-TO PARTY') {
                            $storeCode = $x;
                            $validfile++;
                        } elseif(strtoupper(trim($lineArr[$x]))=='PO' || strtoupper(trim($lineArr[$x]))=='CUSTOMER PO#' || strtoupper(trim($lineArr[$x]))=='PURCHASE ORDER NO.') {
                            $reference = $x;
                            $validfile++;                                 
                        } elseif(strtoupper(trim($lineArr[$x]))=='MATERIAL') {
                            $prodCode = $x;
                            $validfile++;                  
              ...
              if ($docNo <>  $newDocument)   {
                   ...
                   if($dupDocumentNumber == 'N') {
                        if ($newDocument <>  '')   {
                         ...
                         $postingOrdersHoldingTO->vendorReference  = '';
                         $postingOrdersHoldingTO->oldAccount       = trim(preg_replace('/[\x22\x27\x2C\x0A\x0D\x00]/', '', $lineArr[$custAcc]));
                         $postingOrdersHoldingTO->debtorsStoreIdentifier = "";
                         $postingOrdersHoldingTO->salesAgentStoreIdentifier = ''; //same as old acc.
