...
require_once __DIR__ . "/../Riches/RichesConstants.php";
...
$rTO = (new PostExtractDAO($dbConn))->queueAllInvoiced(RichesConstants::PrincipalID,
    ...
    $documentTypeArr,
    $documentStatusArr,
    $fromInvDate='2020-01-10',
    $toInvDate=false,
    $chainUIdIn= $importChain,
    $dataSource=false,
    $capturedBy=false,
...
$seDocs = (new ExtractDAO($dbConn))->getDailyExtractInvoicedOrdersWithParms(RichesConstants::PrincipalID, $recipientUId, $importWarehouse, $chainUIdIn);
...
if (sizeof($errorSEUIdArr) > 0) {
    ...
    $mLST = $messagingDAO->getWarehouseNotificationRecipients(RichesConstants::PrincipalID, $eList);
    ...
    foreach($mLST as $elRow) {
     	  ...
     	  if($storeString <> trim($elRow['depot_uid']) .  trim($elRow['email_addr'])  ) {
             ...
             $messagingDAO = new messagingDAO($dbConn);
             $postingDistributionTO->subject = $messagingDAO->getTemplateOmniImportErrorSubject('Richs - ' . trim($elRow['Warehouse'])); 
