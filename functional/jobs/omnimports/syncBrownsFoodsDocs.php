...
require_once __DIR__ . "/../../../properties/BrownsConstants.php";
...
$reArr = (new BIDAO($dbConn))->getNotificationRecipients(BrownsConstants::PrincipalID, NT_DAILY_EXTRACT_CUSTOM);
if (count($reArr) == 0) {
...
$rTO = (new PostExtractDAO($dbConn))->queueAllInvoiced(BrownsConstants::PrincipalID,
    $recipientUId,
    $inclCancelled = false,
    $documentTypeArr,
    $documentStatusArr,
    $fromInvDate=BrownsConstants::BrownsStartDate,
    $toInvDate=false,
    $chainUIdIn= false,
    $dataSource=false,
    $capturedBy=false,
...
$seDocs = (new ExtractDAO($dbConn))->getDailyExtractInvoicedOrdersWithParms(BrownsConstants::PrincipalID, $recipientUId, '', '');
...
if (!is_array($seDocs) || is_array($seDocs) && count($seDocs) == 0) {
    ...
    $mTResult = $setOmniImportAll->setDocumentConfirm(BrownsConstants::PrincipalID, BrownsConstants::ConfirmEndStatus, DST_INVOICED);
...
if (sizeof($storeIdArr) > 0) {
    $storeSpecialArr = (new MiscellaneousDAO($dbConn))->getPrincipalSpecialFieldValuesMultEntities(BrownsConstants::PrincipalID, 527, implode(",", $storeIdArr), CT_STORE_SHORTCODE, $arrayIndex = "entity_uid");
}
if (sizeof($depotIdArr) > 0) {
    $depotSpecialArr = (new MiscellaneousDAO($dbConn))->getPrincipalSpecialFieldValuesMultEntities(BrownsConstants::PrincipalID, 528, implode(",", $depotIdArr), CT_DEPOT_SHORTCODE, $arrayIndex = "entity_uid");
}
...
$omniApi = new OMNIRestAPI(
    BrownsConstants::OmniHostname,
    BrownsConstants::OmniUsername,
    BrownsConstants::OmniPassword,
    BrownsConstants::OmniLiveCompany
);
...
foreach($grpDocs as $ord) :
    ...
    // $invoiceNumber =  ltrim($ord[0]['document_number']);
    
    $documentNumber =  ltrim($ord[0]['document_number']);
    
    $clientDocNumber = ltrim($ord[0]['client_document_number']);
...
/*    if(trim($row['delivery_instructions']) == "" || $row['delivery_instructions'] == NULL) {
    ...
    $omniOrder = (new OmniSalesOrderObj)                                     
        ...
        // ->setPrinciple("KOS")                                             
        // ->setRepCode("")                                                  
        ->setRevenueAccCode(BrownsConstants::RevenueAccount)                 
    ...
    foreach ($ord as $d) :
        ...
        $ExtDiscountValue = (number_format($documentQty * $d['discount_value'], 2, '.', ''));
        $ExtPrice         = (number_format($d['extended_price'], 2, '', ''));
        $ExtPriceExcl     = (number_format($d['extended_price'], 2, '.', ''));
        $ExtPriceIncl     = (number_format($d['total'], 2, '.', ''));            	
        
          $vatRate          = $d['vat_rate'];
          $vatAmt           = $ExtPriceIncl - $ExtPriceExcl ;        
        
        
        $sendWithPrice = 'Y';       // Price fields will be as above else price not sent at all
...
if (count($successSEUIdArr) > 0) {
    $bIResult = (new PostBIDAO($dbConn))->setSmartEventStatusBulk((implode(",", $successSEUIdArr)), "SUCCESS", "");   // SEts Smart event status to C 
    if ($bIResult->type != FLAG_ERRORTO_SUCCESS) {
    ...
    $setOmniImportAll = new PostBIDAO($dbConn);
    $mTResult = $setOmniImportAll->setOmniImportAll($docUids, $recipientUId, BrownsConstants::ConfirmEndStatus);
...
if (sizeof($errorSEUIdArr) > 0) {
    ...
    $mLST = $messagingDAO->getWarehouseNotificationRecipientsAdditionalParm(BrownsConstants::PrincipalID, $eList, CTD_EDI);
    ...
    foreach($mLST as $elRow) {
     	  ...
     	  if($storeString <> trim($elRow['depot_uid']) .  trim($elRow['email_addr'])  ) {
             ...
             $messagingDAO = new messagingDAO($dbConn);
             $postingDistributionTO->subject = $messagingDAO->getTemplateOmniImportErrorSubject('Browns - ' . trim($elRow['Warehouse'])); 
...
$setOmniImportAll = new PostBIDAO($dbConn);    //    Only do this if final status is invoiced
$mTResult = $setOmniImportAll->setDocumentConfirm(BrownsConstants::PrincipalID, BrownsConstants::ConfirmEndStatus, DST_INVOICED);
