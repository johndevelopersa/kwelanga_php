...
require_once __DIR__ . "/../../../properties/RhodesConstants.php";
...
//       echo "<br>";
       ...
       if($orderStore <> $row['dm_uid']) {   // Set Up Header
              $storeSpecialArr = (new MiscellaneousDAO($dbConn))->getPrincipalSpecialFieldValuesMultEntities(RhodesConstants::PrincipalID, 514, $row['principal_store_uid'], CT_STORE_SHORTCODE, $arrayIndex = FALSE);

              $depotSpecialArr = (new MiscellaneousDAO($dbConn))->getPrincipalSpecialFieldValuesMultEntities(RhodesConstants::PrincipalID, 516, $row['depot_uid'], CT_DEPOT_SHORTCODE, $arrayIndex = FALSE);
...
OmniErrorReporting(RhodesConstants::PrincipalID, $row['type_uid'], CTD_EDI);

OmniErrorReporting(RhodesConstants::PrincipalID, $row['type_uid'], CTD_ADMIN_CLERK);
...
// ******************************************************************************************************************************
 function WriteToOmni($omniOrder, $documentNumber, $DocUid, $setype, $updateSEstatus="", $seUID) {
     ...
     if (!isset($dbConn)) {
 	   ...
 	   $omniApi = new OMNIRestAPI(RhodesConstants::OmniHostname,
...
*/        
    ...
    if($updateSEstatus == FLAG_STATUS_CLOSED ) {
           $setOmniImportAll = new PostBIDAO($dbConn);
           $mTResult = $setOmniImportAll->setOmniUpdateTrans($DocUid, $setype, RhodesConstants::ConfirmEndStatus);
    }
