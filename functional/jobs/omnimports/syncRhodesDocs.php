...
$reArr = (new BIDAO($dbConn))->getNotificationRecipients(RhodesConstants::PrincipalID, NT_DAILY_EXTRACT_CUSTOM);
if (count($reArr) == 0) {
...
$rTO = (new PostExtractDAO($dbConn))->queueAllInvoiced(RhodesConstants::PrincipalID,
    $recipientUId,
    $inclCancelled = false,
    $documentTypeArr,
    $documentStatusArr,
    $fromInvDate=RhodesConstants::RhodesStartDate,
    $toInvDate=false,
    $chainUIdIn= false,
    $dataSource=false,
    $capturedBy=false,
...
$seDocs = (new ExtractDAO($dbConn))->getDailyExtractInvoicedOrdersWithParms(RhodesConstants::PrincipalID, $recipientUId, '', '');
...
if (!is_array($seDocs) || is_array($seDocs) && count($seDocs) == 0) {
    ...
    $mTResult = $setOmniImportAll->setDocumentConfirm(RhodesConstants::PrincipalID, RhodesConstants::ConfirmStartStatus, RhodesConstants::ConfirmEndStatus);
...
if (sizeof($storeIdArr) > 0) {
    $storeSpecialArr = (new MiscellaneousDAO($dbConn))->getPrincipalSpecialFieldValuesMultEntities(RhodesConstants::PrincipalID, 514, implode(",", $storeIdArr), CT_STORE_SHORTCODE, $arrayIndex = "entity_uid");
}
if (sizeof($depotIdArr) > 0) {
    $depotSpecialArr = (new MiscellaneousDAO($dbConn))->getPrincipalSpecialFieldValuesMultEntities(RhodesConstants::PrincipalID, 516, implode(",", $depotIdArr), CT_DEPOT_SHORTCODE, $arrayIndex = "entity_uid");
}
...
$omniApi = new OMNIRestAPI(
    RhodesConstants::OmniHostname,
    RhodesConstants::OmniUsername,
    RhodesConstants::OmniPassword,
    RhodesConstants::OmniLiveCompany
);
...
foreach($grpDocs as $ord) :
    ...
    $omniOrder = (new OmniSalesOrderObj)                                     
        ...
        // ->setPrinciple("KOS")                                             
        // ->setRepCode("")                                                  
        ->setRevenueAccCode(RhodesConstants::RevenueAccount)                 
...
if (count($successSEUIdArr) > 0) {
    ...
    $setOmniImportAll = new PostBIDAO($dbConn);
    $mTResult = $setOmniImportAll->setOmniImportAll($docUids, $recipientUId, RhodesConstants::ConfirmEndStatus);
...
if (sizeof($errorSEUIdArr) > 0) {
    ...
    $mLST = $messagingDAO->getWarehouseNotificationRecipientsAdditionalParm(RhodesConstants::PrincipalID, $eList, CTD_EDI);
    ...
    foreach($mLST as $elRow) {
     	  ...
     	  if($storeString <> trim($elRow['depot_uid']) .  trim($elRow['email_addr'])  ) {
             ...
             $messagingDAO = new messagingDAO($dbConn);
             $postingDistributionTO->subject = $messagingDAO->getTemplateOmniImportErrorSubject('Rhodes - ' . trim($elRow['Warehouse'])); 
