...
require_once __DIR__ . "/../../../properties/Omni_Constants_393.php";
...
if (sizeof($storeIdArr) > 0) {
    $storeSpecialArr      = (new MiscellaneousDAO($dbConn))->getPrincipalSpecialFieldValuesMultEntities($constantsClass::PrincipalID, 522, implode(",", $storeIdArr), CT_STORE_SHORTCODE, $arrayIndex = "entity_uid");
    $omniBranchSpecialArr = (new MiscellaneousDAO($dbConn))->getPrincipalSpecialFieldValuesMultEntities($constantsClass::PrincipalID, 524, implode(",", $storeIdArr), CT_STORE_SHORTCODE, $arrayIndex = "entity_uid");
}
...
/*        $createdOrder = $omniApi->GetSalesOrder($documentNumber);
        ...
        if(count($warnings)){
            ...
            echo "[WARNINGS] " . $ord[0]['smartUid'] . " ====> " . implode(";", $warnings);

            $bIResult = (new PostBIDAO($dbConn))->setSmartEventStatusBulk($ord[0]['smartUid'], "WARNING", implode(";", $warnings));
