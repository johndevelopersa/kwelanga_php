...
require_once __DIR__ . "/../rollingchicken/RolingChickenConstants.php";
...
$reArr = (new BIDAO($dbConn))->getNotificationRecipients(RolingChickenConstants::PrincipalID, NT_DAILY_EXTRACT_CUSTOM);
if (count($reArr) == 0) {
...
$rTO = (new PostExtractDAO($dbConn))->queueAllInvoiced(RolingChickenConstants::PrincipalID,
    $recipientUId,
    $inclCancelled = false,
    $documentTypeArr,
    $documentStatusArr,
...
$seDocs = (new ExtractDAO($dbConn))->getDailyExtractInvoicedOrdersWithParms(RolingChickenConstants::PrincipalID, $recipientUId, $importWarehouse, $altChainUIdIn);
...
if (!is_array($seDocs) || is_array($seDocs) && count($seDocs) == 0) {
    ...
    $mTResult = $setOmniImportAll->setDocumentConfirm(RolingChickenConstants::PrincipalID, DST_ACCEPTED, DST_INVOICED);
...
if (sizeof($storeIdArr) > 0) {
    $storeSpecialArr = (new MiscellaneousDAO($dbConn))->getPrincipalSpecialFieldValuesMultEntities(RolingChickenConstants::PrincipalID, 481, implode(",", $storeIdArr), CT_STORE_SHORTCODE, $arrayIndex = "entity_uid");
}
if (sizeof($depotIdArr) > 0) {
    $depotSpecialArr = (new MiscellaneousDAO($dbConn))->getPrincipalSpecialFieldValuesMultEntities(RolingChickenConstants::PrincipalID, 480, implode(",", $depotIdArr), CT_DEPOT_SHORTCODE, $arrayIndex = "entity_uid");
}
...
$omniApi = new OMNIRestAPI(
    RolingChickenConstants::OmniHostname,
    RolingChickenConstants::OmniUsername,
    RolingChickenConstants::OmniPassword,
    RolingChickenConstants::OmniLiveCompany
);
...
foreach($grpDocs as $ord) :
    ...
    if($altChainUIdIn == 2837) {
        $documentNumber =  ltrim($ord[0]['document_number']);
        $clientDocNumber = ltrim($ord[0]['client_document_number']);
    } else {
         $documentNumber =  'RC' . ltrim($ord[0]['document_number'],'0');
         $clientDocNumber = 'RC' . ltrim($ord[0]['client_document_number'],'0');
    }
    ...
    if(trim($ord[0]['invoice_number']) == '') {
         if($altChainUIdIn == 2837) {
             $invoiceNumber =  ltrim($ord[0]['document_number']);
         	    ...
         	    $invoiceNumber = 'RC' . ltrim($ord[0]['document_number'],'0');
         }    
    } else {
         if($altChainUIdIn == 2837) {
             $invoiceNumber =  ltrim($ord[0]['document_number']);
...
if (sizeof($errorSEUIdArr) > 0) {
    ...
    $mLST = $messagingDAO->getWarehouseNotificationRecipientsAdditionalParm(RolingChickenConstants::PrincipalID, $eList, CTD_EDI);
    ...
    foreach($mLST as $elRow) {
     	  ...
     	  if($storeString <> trim($elRow['depot_uid']) .  trim($elRow['email_addr'])  ) {
             ...
             $messagingDAO = new messagingDAO($dbConn);
             $postingDistributionTO->subject = $messagingDAO->getTemplateOmniImportErrorSubject('Rolling Chicken - ' . trim($elRow['Warehouse'])); 
