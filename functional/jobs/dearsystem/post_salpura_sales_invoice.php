<?php
// Naomi 0637772326
// https://kwelangaonlinesolutions.co.za/systems/kwelanga_system/kwelanga_php/functional/jobs/dearsystem/post_salpura_sales_invoice.php
...
$apiBaseUri     = SalpuraContants::DearHostname;
$accountId      = SalpuraContants::DearUsername;
$applicationKey = SalpuraContants::DearPassword;
$PrincipalID    = SalpuraContants::PrincipalID;
$NotifyId       = SalpuraContants::extractType;
/*-------------------------------------------------*/
 ...
 *-----------------------------------------*/ 
     ...
     foreach($seDocs as $orow) {
           ...
           // Process Header
           if($header == 'T') {
               $storeSpecialArr       = (new MiscellaneousDAO($dbConn))->getPrincipalSpecialFieldValuesMultEntities(SalpuraContants::PrincipalID, 590, $orow['psmUid'], CT_STORE_SHORTCODE, $arrayIndex = "entity_uid");
               $royalBranchSpecialArr = (new MiscellaneousDAO($dbConn))->getPrincipalSpecialFieldValuesMultEntities(SalpuraContants::PrincipalID, 591, $orow['psmUid'], CT_STORE_SHORTCODE, $arrayIndex = "entity_uid");
               ...
               if (empty($storeSpecialArr[$orow['psmUid']]['value'])) {  //has no special field and/or blank...
                       ...
                       $updateGeneral1 = '555';
                       $general2       = '';
                       $statusMsg  = "[KOS] Missing Salpura SPF for store:   <br>";                   
               ...
               if (empty($royalBranchSpecialArr[$orow['psmUid']]['value'])) {  //has no special field and/or blank...
                       ...
                       $updateGeneral1 = '556';
                       $general2       = '';
                       $statusMsg  = "[KOS] Missing Salpura spf for location:   <br>";                   
                       $setOmniImportAll = new PostBIDAO($dbConn);
                       $mTResult = $setOmniImportAll->setSmartEventStatusIndividual($orow['smartUid'], $updateGeneral1, $general2 , $statusMsg , $updateSEstatus );
                       $processErrors = 'Y';
                       echo "Return Account Error";
