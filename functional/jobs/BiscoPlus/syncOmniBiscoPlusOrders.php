...
$statusArr = (array($constantsClass::StartStatus));
...
//   print_r($omniOrder->getJSON());
    ...
    if (!$response->getSuccess() || $response->getSuccess() && $responseOrderNo != $documentNumber) {
        ...
        if (strpos(strtolower($errMessage), "already exists") !== false) {
            ...
            $errorKey = "[APIResponse]: " . $errMessage;
            if (!isset($errorSEUIdArr[$errorKey])) {
                $errorSEUIdArr[$errorKey] = [];
            }
            $errorSEUIdArr[$errorKey][] = $ord[0]['smartUid']; //list of smart event success

            //Split Business Errors or Technical Errors
            echo str_repeat("-", 45) . "\n";
            if (strpos($errorKey, "Customer Account Code") !== false && strpos($errorKey, "on Hold") !== false) {
                echo "BUSINESS ISSUE: Account On Hold: \n" . $errorKey . "\n";
            } else if (strpos($errorKey, "Invalid Customer Account Code") !== false) {
                echo "BUSINESS ISSUE: Invalid Customer Account Code: \n" . $errorKey . "\n";
            } else {
                echo "Technical Error Creating Order: " . $errorKey . "\n";
        ...
        echo "<h1>[OMNI] Successfully Created Order:" . $responseOrderNo . "</h1>";
    }
    break;
}
...
if (count($successSEUIdArr) > 0) {
    ...
    $uSE = $updateImportStat->setOmniImportStatus(implode(",", $successSEUIdArr), $constantsClass::extractType, DST_ACCEPTED);
}
...
if (sizeof($errorSEUIdArr) > 0) {
    ...
    echo "generating errors... ".count($errorSEUIdArr)."\n<br>";
    ...
    OmniErrorReporting($constantsClass::PrincipalID, $seDocs[0]['type_uid'], CTD_EDI, 'Orders');
}
...
// ******************************************************************************************************************************
function OmniErrorReporting($principalUId, $seType, $nType, $subjectPrefix = '', $resendMinutes = 60)
{
    ...
    if (count($errLST) > 0) {
        ...
        foreach($errLST as $elRow){
            unset($elRow['error_count']);
            $uniqueData = json_encode($errLST);
        }
        ...
        foreach ($errLST as $elRow) {
            ...
            if ($storeString <> trim($elRow['email_addr'])) {
                $messagingDAO = new messagingDAO($dbConn);
                $postingDistributionTO->subject = $messagingDAO->getTemplateOmniImportErrorSubject(trim($subjectPrefix . ' ' . $elRow['Principal']));
        ...
        $temp_file = sys_get_temp_dir() . "/omni_error_{$principalUId}_" . md5($uniqueData);

        if(is_file($temp_file)){
            $age = time() - filemtime($temp_file);
            echo "file age: $age\n";
            if($age < $resendMinutes * 60) {
                echo "error seen within 60min ago, not generating again\n";
                return;
            }

            echo "error seen more than 60min ago, generating again\n";
        }  else {
            echo 'first time seeing error.';
