...
$reArr = (new BIDAO($dbConn))->getNotificationRecipients($constantsClass::PrincipalID, NT_DAILY_EXTRACT_ALTCUSTOM2);
if (count($reArr) == 0) {
...
$statusArr = (array($constantsClass::StartStatusCredits));

$rTO = (new PostExtractDAO($dbConn))->queueAllCreditsAndDebits($constantsClass::PrincipalID,
                                                               $recipientUId,
                                                               $inclCancelled = false,
                                                               $depotUId = false,
                                                               $documentTypeArr,
                                                               $fromInvDate='2023-06-01',
                                                               $toInvDate= false,
                                                               $chainUIdIn= false,
                                                               $dataSource=false,
                                                               $capturedBy=false,
...
$seDocs = (new OmniExtractDAO($dbConn))->getOrdersForOmni($constantsClass::PrincipalID, 
                                                          $constantsClass::extractTypeCredits,
                                                          '', 
...
foreach($grpDocs as $ord) {
    ...
    if(trim($ord[0]['alternate_document_number']) == '') {
    	  $creditNoteNumber = ltrim($ord[0]['document_number'],'0');
    } else {
    	  $creditNoteNumber = ltrim($ord[0]['alternate_document_number'],'0');
    }   
    $customerOrderNumber = $ord[0]['customer_order_number'];
    $customerClaimNumber = $ord[0]['claim_number'];
    $branchCode          = $ord[0]['branch_code'];   
    $documentStatus      = $ord[0]['document_status_uid'];
    $orderDate           = $ord[0]['customer_order_number'];
    $sourceDocNo         = $ord[0]['source_document_number'];
...
//    $billAdd3 = substr($ord[0]["bill_add3"],0,30);
    ...
    $omniOrder = (new OmniCreditNoteObj)
        ...
        ->setDeliveryDetails($sourceDocNo)  //  *************
        //->setDeliveryRoute()
...
//      $prodRevenueCode = $d['prod_group_code'];
        ...
        $batArr = $getBatchNo->getCreditNoteProductBatch($constantsClass::PrincipalID,
                                                         $sourceDocNo,
                                                         $d['pp.uid']);
...
//      print_r($batArr);
        ...
        /*-------------------------------------------------------------
          *      CREATE ORDER LINES
          *-----------------------------------------------------------*/
        $orderLineItem = (new OmniCreditNoteLineItemObj )
...
//print_r($omniOrder->getJSON());
     ...
     if($loadInvoice == 'T'){
     	   ...
     	   $response = $omniApi->CreateCreditNote($omniOrder,$creditNoteNumber);
