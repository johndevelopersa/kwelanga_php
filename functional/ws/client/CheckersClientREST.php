...
class CheckersClientREST
{
    ...
    public function runProcess_getOrders($principalUId, $vendorAccount, $username, $password, $vendorSystem)
    {
    ...
    public function Sendrequestfororders($username, $password, $vendorSystem)
    {
        // URL_CHECKERS
        
    	  if($vendorSystem == 'QA') {
               return $this->WebServiceCall($logFolder = "checkers-orders-request-log", $endPoint = WS_URL_CHECKERS_ORDERS_REST_TEST . "VendorOrder", $username, $password, CURLOPT_HTTPGET);
        } else {        	
               return $this->WebServiceCall($logFolder = "checkers-orders-request-log", $endPoint = WS_URL_CHECKERS_ORDERS_REST . "VendorOrder", $username, $password, CURLOPT_HTTPGET);	
        }
    }
    ...
    {
        if($vendorSystem == 'QA') {
               return $this->WebServiceCall($logFolder = "checkers-claims-request-log", $endPoint = WS_URL_CHECKERS_CLAIMS_REST_TEST . "VendorClaim", $username, $password, CURLOPT_HTTPGET);
        } else {
               return $this->WebServiceCall($logFolder = "checkers-claims-request-log", $endPoint = WS_URL_CHECKERS_CLAIMS_REST . "VendorClaim", $username, $password, CURLOPT_HTTPGET);
        }
    }

    public function sendOrderConfirmation($username, $password, $orderList, $vendorSystem)
    {
        // replace values
        $sendJSON = '[' . $orderList . ']';
        
        if($vendorSystem == 'QA') {
               $endPoint = WS_URL_CHECKERS_ORDERS_REST_TEST . "VendorOrder?action=Acknowledge";
        } else {
               $endPoint = WS_URL_CHECKERS_ORDERS_REST . "VendorOrder?action=Acknowledge";
        }

        return $this->WebServiceCall($logFolder = "checkers-orders-confirmation-log",
            $endPoint,
            $username,
            $password,
            CURLOPT_PUT,
    ...
    public function sendClaimConfirmation($username, $password, $claimList)
    {
        ...
        if($vendorSystem == 'QA') {
           $endPoint = WS_URL_CHECKERS_CLAIMS_REST_TEST . "VendorClaim?action=Acknowledge";
        } else {
           $endPoint = WS_URL_CHECKERS_CLAIMS_REST . "VendorClaim?action=Acknowledge";
        }

        return $this->WebServiceCall($logFolder = "checkers-claims-log",
            $endPoint,
            $username,
            $password,
            CURLOPT_POST,
    ...
    function setupTO_orders($principalUId, $vendorAccount, &$orders)
    {
        ...
        $logResult = NewRelic::logEvent(
            $logType = "checkers-orders-hw",
            $script = basename(__FILE__),
            $msg = print_r($orders, TRUE),
            $attr = [
        ...
        if (!$logResult) {
            echo "Failed to create log event for CHECKERS ORDERS HW";
        }
        ...
        foreach ($orders["order"] as &$o) {
            ...
            if (!$logResult) {
                echo "Failed to create log event for CHECKERS ORDERS -O";
            }
            ...
            $postingOrdersHoldingTO->additionalType = $o['orderTypeCode'];

            echo $o['orderTypeCode'];

            $storeName = 'F';

            if ($o['orderTypeCode'] <> '220' && $o['orderTypeCode'] <> '258') {    // 258
                ...
                // 6001001360203 SH/CH CENTURION DC 36021
                // 6001001038706 Exports DC Centurion      3870

                $shrinkCustomers = array("6001001360203", "6001001038706");

                if ($principalUId == 71 && in_array($o["buyer"]["gln"], $shrinkCustomers)) {
                    $shrink = 'N';
                }
                $storeName = 'T';
            }
            ...
            foreach ($o["orderLineItem"] as $ol) {
                ...
                if ($storeName = 'F') {
                    $postingOrdersHoldingTO->deliverName = $ol["orderLineItemDetail"]["orderLogisticalInformation"]["shipTo"]["additionalPartyIdentification"]["0"] . "   " . $ol["orderLineItemDetail"]["orderLogisticalInformation"]["shipTo"]["additionalPartyIdentification"]["1"];
                    $postingOrdersHoldingTO->shipToName = $postingOrdersHoldingTO->deliverName;

                    $postingOrdersHoldingTO->oldAccount = "";
                    $postingOrdersHoldingTO->shipToGLN = $ol["orderLineItemDetail"]["orderLogisticalInformation"]["shipTo"]["gln"];
                    if (trim($postingOrdersHoldingTO->shipToGLN) == "") {
                        // do not allow to continue - reject the file and ITD will be notified in confirmation
                        echo "The EAN / GLN is blank for order ({$postingOrdersHoldingTO->reference}) vendor account ({$vendorAccount}) in CheckersClient for principal {$principalUId} @" . date(GUI_PHP_DATETIME_FORMAT);
                        continue;
                ...
                if ($ol["orderLineItemDetail"]["orderLogisticalInformation"]["orderLogisticalDateInformation"]["requestedDeliveryDateTime"]["date"] != "")
                    $postingOrdersHoldingTO->requestedDeliveryDate = $ol["orderLineItemDetail"]["orderLogisticalInformation"]["orderLogisticalDateInformation"]["requestedDeliveryDateTime"]["date"];
                 ...
                 */
                if ($shrink == 'Y') {
                    $postingOrdersHoldingDetailTO->productSKUGTIN = preg_replace("/^[0]+/", "", $ol["transactionalTradeItem"]["gtin"]) . 'S'; // Checkers use innercasing GTIN
                    $postingOrdersHoldingDetailTO->productGTIN = preg_replace("/^[0]+/", "", $ol["transactionalTradeItem"]["gtin"]) . 'S'; // Checkers use innercasing GTIN
                } else {
    ...
    function setupTO_claims($principalUId, $vendorAccount, &$claims)
    {
        ...
        $logResult = NewRelic::logEvent(
            $logType = "checkers-claims",
            $script = basename(__FILE__),
            $msg = print_r($claims, TRUE),
            $attr = [
                'principal_uid' => $principalUId,
                'vendor_account' => $vendorAccount,
                'timestamp' => gmdate('YmdHis'),
        ...
        if (!$logResult) {
            echo "Failed to create log event for CHECKERS CLAIMS";
        }
        ...
        foreach ($claims["claimsNotification"] as $key1 => &$c) {
            ...
            $postingOrdersHoldingTO->wsUniqueCreatorId = $c["claimsNotificationIdentification"]["contentOwner"]['additionalPartyIdentification'][0];
            ...
            if (strtoupper(strval($c["supplementalMessageDescription"])) == 'RETURN') {
                $claimtype = "11_RETURNS_DAMAGE";
...
//            $postingOrdersHoldingTO->sourceDocumentNo = $c["purchaseOrder"]["entityIdentification"]; // creationDateTime is the date field
            } elseif (strtoupper(strval($c["supplementalMessageDescription"])) == 'SHORTAGE') {
                $claimtype = "A4_NON_RECEIPT_OF_GOODS";
                $postingOrdersHoldingTO->sourceDocumentNo = $c["claimsNotificationIdentification"]["entityIdentification"];
                $postingOrdersHoldingTO->clientDocumentNo = $c["claimsNotificationIdentification"]["entityIdentification"];
//            $postingOrdersHoldingTO->reference        = $c["purchaseOrder"]["entityIdentification"]; // creationDateTime is the date field
            } elseif (strtoupper(strval($c["supplementalMessageDescription"])) == 'PRICING') {
                $claimtype = "01_PRICING_ERROR";
                $postingOrdersHoldingTO->sourceDocumentNo = $c["claimsNotificationIdentification"]["entityIdentification"];
                $postingOrdersHoldingTO->clientDocumentNo = $c["claimsNotificationIdentification"]["entityIdentification"];
//            $postingOrdersHoldingTO->reference        = $c["purchaseOrder"]["entityIdentification"]; // creationDateTime is the date field
            } elseif (strtoupper(strval($c["supplementalMessageDescription"])) == 'OVERCHARGE') {
                $claimtype = "01_PRICING_ERROR";
                $postingOrdersHoldingTO->sourceDocumentNo = $c["claimsNotificationIdentification"]["entityIdentification"];
                $postingOrdersHoldingTO->clientDocumentNo = $c["claimsNotificationIdentification"]["entityIdentification"];
//            $postingOrdersHoldingTO->reference        = $c["purchaseOrder"]["entityIdentification"]; // creationDateTime is the date field
            } else {
                $claimtype = "UNKNOWN CLAIM TYPE";
                $postingOrdersHoldingTO->sourceDocumentNo = $c["claimsNotificationIdentification"]["entityIdentification"];
                $postingOrdersHoldingTO->clientDocumentNo = $c["claimsNotificationIdentification"]["entityIdentification"];
//            $postingOrdersHoldingTO->reference        = $c["purchaseOrder"]["entityIdentification"]; // creationDateTime is the date field
            }

            $postingOrdersHoldingTO->additionalType = $claimtype;
...
//          $postingOrdersHoldingDetailTO->productCode = $dtl["expectedToReceive"]["transactionalTradeItem"]["additionalTradeItemIdentification"][1]; // vs ["actualReceived"]
                ...
                $postingOrdersHoldingDetailTO->productGTIN = preg_replace("/^0*/", "", $dtl["expectedToReceive"]["transactionalTradeItem"]["gtin"]); // ["additionalTradeItemIdentification"][1] contains outerGTIN


                $postingOrdersHoldingDetailTO->productName = $dtl["expectedToReceive"]["transactionalTradeItem"]["tradeItemDescription"];
                if ($claimtype == '11_RETURNS_DAMAGE') {
                    $postingOrdersHoldingDetailTO->quantity = $dtl["actualReceived"]["quantity"];
                } elseif ($claimtype == 'A4_NON_RECEIPT_OF_GOODS') {
                    $postingOrdersHoldingDetailTO->quantity = $dtl["actualReceived"]["quantity"] - $dtl["expectedToReceive"]["quantity"];
                } else {
                    $postingOrdersHoldingDetailTO->quantity = $dtl["actualReceived"]["quantity"];
                }
                // $dtl["expectedToReceive"]["price"] is excl vat and is extPrice
                $postingOrdersHoldingDetailTO->listPrice = ((abs($postingOrdersHoldingDetailTO->quantity) > 0) ? round($dtl["claimAmount"] / $postingOrdersHoldingDetailTO->quantity, 3) : "");
                $postingOrdersHoldingDetailTO->nettPrice = $postingOrdersHoldingDetailTO->listPrice;
                $postingOrdersHoldingDetailTO->extPrice = $dtl["claimAmount"];
    ...
    public function runProcess_uploadInvoice()
    {
        ...
        if (count($rs) == 0) {
            ...
            // checkers upload supports multiple \ in one call, but for time being leave as 1 per upload as im not sure what will happen if 1 out of 20 fails
            foreach ($rs["header"] as $doc) {

                $username     = $doc["username"];
                $password     = $doc["password"];
                $vendorSystem = $doc["vendor_system"];

                $seTO = new SmartEventTO();

                $seTO->type = SE_INVOICE_UPLOAD_REST;
    ...
    public function UploadInvoice($header, $detail, $username, $password, $vendorSystem)
    {
        global $ROOT, $PHPFOLDER;
        ...
        $trimArray = array('6005188000006', '6009668780006', '6002107000000', '1138774000007', '6009900479200', '6001007802929');


        if (in_array($header["principal_gln"], $trimArray)) {
            $invoiceNumber = ltrim(trim(((empty($header["invoice_number"])) ? $header["document_number"] : $header["invoice_number"])), "0");
        } else {
        ...
        if (trim($header["ean_code"]) == "") $errors[] = "Store GLN not supplied ({$header["principal_store_uid"]}  {$header["bill_name"]})";
        ...
        $sendXML = str_replace(array("&&var_uploadDateTime&&",
            ...
            array($now,
                ...
                round($header["invoice_total"],4),
                round($header["exclusive_total"],4),
                round($header["vat_total"],4),
        ...
        foreach ($detail as $row) {
            ...
            if (trim($row["outercasing_gtin"]) <> '') {
                $invoicebarcode = str_replace('S', '', $row["outercasing_gtin"]);
            } else {
                $invoicebarcode = "";
            }

            if (!preg_match("/^[0-9]{8,}$/", trim($invoicebarcode))) $errors[] = "Invalid Product ({$row["product_code"]}) GTIN for {$header["principal_name"]}, Doc {$header["document_number"]}, PO {$header["customer_order_number"]}";


            $weight = "0"; // ($row["weight"]) Checkers might have same rule as PnP which requires that if we send a weight (ours are all "1" anyway), then they require a proper unitOfMeasure, so we don't send weight

            if (trim($row["packing"]) == 'KG') {
                $uom = 'KG';
            } else {
            	  if($row["items_per_case"] == '1') {
                    $uom = "EA";
            	  } else {
                    $uom = "CA";
            	  }
            ...
            $dtlArr[] = str_replace(array("&&var_lineNumber&&",
                ...
                array($row["line_no"],
                    ...
                    $row["document_qty"],
                    $deliveryDate,
                    round($row["net_price"],4),
                    str_replace("&", "", $row["product_description"]),
                    round($row["net_price"],4),
                    round($row["net_price"] * (1 + $row["vat_rate"] / 100),4),
                    round($row["vat_amount"] / $row["document_qty"],4),
                    $row["vat_rate"],
        ...
        $sendXML = str_replace("&&var_invoiceDetailLines&&", implode("", $dtlArr), $sendXML);
         // echo "H";
         // $logfile = "Checkers_Invoices" . date('ymd') . 'T.txt';      
         // file_put_contents($ROOT.$PHPFOLDER.'log/' . $logfile, $sendXML, FILE_APPEND);

        $logResult = NewRelic::logEvent(
            $logType = "checkers-invoices",
            $script = basename(__FILE__),
            $msg = $sendXML,
            $attr = [
                'principal_gln' => $header["principal_gln"] ?? '',
        ...
        if (!$logResult) {
            echo "Failed to create log event for CHECKERS INVOICE or options not passed";
        }
        
        if($vendorSystem == 'QA') {
              $endPoint = WS_URL_CHECKERS_INVOICE_REST_TEST . "VendorInvoice";
        } else {
              $endPoint = WS_URL_CHECKERS_INVOICE_REST . "VendorInvoice";	
        }
        return $this->WebServiceCall($logFolder = "checkers-invoice-log",
            $endPoint,
            $username,
            $password,
            CURLOPT_POST,
