...
class nusoap_fault extends nusoap_base {
	...
	*/
	function nusoap_fault($faultcode,$faultactor='',$faultstring='',$faultdetail='',$faultheader=''){

		parent::nusoap_base();
		$this->faultheader = $faultheader; 	//NEW
	  ...
	  global $HTTP_RAW_POST_DATA;
		WSlogger::wsServerLog('FAULT', 'NUSP: '.$faultactor.', '.str_replace("\n",'',str_replace("\r",'',$faultstring)).', '.str_replace("\n",'',str_replace("\r",'',$faultdetail)),null,null,null,$HTTP_RAW_POST_DATA);
	}
	...
	*/
	function serialize(){
		...
		*/
		...
		if($this->faultheader!=''){
			$return_msg .= '<SOAP:Header>'."\r\n".
			$this->faultheader.
			'</SOAP:Header>'."\r\n";
		}

				$return_msg .='<SOAP:Body>'."\r\n".
				'<SOAP:Fault>'."\r\n".
...
//		}
        ...
        if (is_array($v)) $err .= "$k: ".implode(",",$v)."<br>"; // modified by Mark 17May2013 as some elements are array and not string
				else $err .= "$k: $v<br>"; // modified by Mark 17May2013 as some elements are array and not string
			}
			$this->debug($err);
			$this->setError($err);
			curl_close($this->ch);
		...
		// Added this block by Mark : take out checkers 307-moved temporarily header so it can parse
		if (
		 (preg_match("/^HTTP.+ 2[0-9][0-9] /msU",$data)) &&
		 (preg_match("/^HTTP.+ 307 Moved Temporarily/msU",$data))
		 ) {
		 $data = preg_replace("/HTTP\/1.1 307 Moved Temporarily.+HTTP\/1.1 200 OK/msU", "HTTP/1.1 200 OK", $data);
		}
		...
		if(strlen($data) == 0){
			...
			$this->setError('no data present after HTTP headers in the response');
			return false;
		}
...
class nusoap_server extends nusoap_base {
	...
	function serialize_return() {
		...
		if ($this->wsdl) {
			...
			if ($this->opData['style'] == 'rpc') {
				...
				if(strpos($return_val,'<>')!==false&&strpos($return_val,'</>')!==false){
					$return_val = substr(substr($return_val,2,strlen($return_val)),0,-3);
				}

				if ($this->opData['output']['use'] == 'literal') {
					// http://www.ws-i.org/Profiles/BasicProfile-1.1-2004-08-24.html R2735 says rpc/literal accessor elements should not be in a namespace
					if ($this->methodURI) {
						$payload = '<sh:'.$this->methodname.' xmlns:sh="http://www.unece.org/cefact/namespaces/StandardBusinessDocumentHeader" xmlns:SOAP="http://schemas.xmlsoap.org/soap/envelope/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.unece.org/cefact/namespaces/StandardBusinessDocumentHeader">'.$return_val.'</sh:'.$this->methodname.">";
					} else {
			...
			$payload = '<sh:'.$this->methodname.'Response xmlns:ns1="'.$this->methodURI.'">'.$return_val.'</ns1:'.$this->methodname."Response>";
		}
	...
	*/
	function send_response() {
		...
		print html_entity_decode($payload);
	...
	*/
    function parseRequest($headers, $data) {
      ...
      $this->requestArray = $parser->message;
		}
	 }
...
class nusoap_client extends nusoap_base  {
  ...
  var $incoming_headers = NULL; // Added by Mark 25-Mar-2013 for Checkers implementation
        ...
        $usedNamespaces = $this->wsdl->usedNamespaces;
			...
			if (is_string($params)) {
				...
				foreach($params as $k => $v){
					$payload .= $this->serialize_val($v,$k,false,false,false,array('xmlns:xsi'=>'http://www.w3.org/2001/XMLSchema-instance'),$use);
				}
			} else {
				$this->debug('params must be array or string');
				$this->setError('params must be array or string');
	...
	*/
	function send($msg, $soapaction = '', $timeout=0, $response_timeout=30) {
        ...
        $this->incoming_headers = $http->incoming_headers; // Added by Mark 25-Mar-2013
