<?php
/*
 https://kwelangaonlinesolutions.co.za/systems/kwelanga_system/kwelanga_php/functional/integrations/xero/pushTrigzCredits.php";
...
$principalId = 439;
$principalStoreSpecialFieldID = 599;	/* !!PENTACORP CATCH : Xero Contact ID!! */
/*-----------------------------------*/
...
/*-------------------------------------------------*/
$reArr = (new BIDAO($dbConn))->getNotificationRecipients($principalId, NT_DAILY_EXTRACT_ALTCUSTOM2);
if (count($reArr)==0) {
...
$documentTypeArr = [
    DT_CREDITNOTE, DT_MCREDIT_PRICING, DT_MCREDIT_OTHER
];
...
/*-------------------------------------------------*/
$rTO = (new PostExtractDAO($dbConn))->queueAllCreditsAndDebits($principalId,
                                                               $recipientUId,
                                                               $inclCancelled = false,
                                                               $depotUId = false,
                                                               $documentTypeArr,
...
if (!is_array($seDocs) || is_array($seDocs) && count($seDocs)==0) {
    echo "Successful --> No outstanding Credits found!";
    return;
}
...
// print_r($lineArr);
     ...
     strlen(trim($storeSpecialArr[$storeId]['value'])) != 36)	//xero ids are 36 chars
  {
	...
	$chainXeroContactID = trim($storeSpecialArr[$storeId]['value']);
	...
	foreach($lineArr as $lineRow){
  	   ...
  	   if($lineRow['document_qty'] == 0) {
  	   	    continue;
  	   }
  	
       if(strlen($lineRow['product_code']) > 8) {
            $productCode = $lineRow['alt_code'];			
       } else {
            $productCode = $lineRow['product_code'];	
       }
       $productDesc        = $lineRow['product_description'];
       $productUnitAmount  = round($lineRow['net_price'] / $lineRow['items_per_case'],2) ;	//this could vary across invoices (times and sessions)
       $productTotalQty    = abs($lineRow['document_qty'] * $lineRow['items_per_case']);
       $productTotalAmount = round($productTotalQty * $productUnitAmount,2);
       $productTotalTax    = round($productTotalAmount * 0.15,2);
	...
	$invoice = new XeroAPI\XeroPHP\Models\Accounting\CreditNote;
	$invoice->setType(XeroAPI\XeroPHP\Models\Accounting\CreditNote::TYPE_ACCRECCREDIT)
			->setReference(ltrim($lineArr[0]['document_number'],'0') . ' - ' . trim($lineArr[0]['customer_order_number']) . ' - ' . trim($lineArr[0]['branch_code']) . ' - ' . trim($lineArr[0]['Area']))
		  ->setCreditNoteNumber($lineArr[0]['alternate_document_number'])
			//->setAmountDue()
			//->setAmountPaid()
			//->setTotalDiscount()
			//->setTotalTax()
			...
			->setStatus(XeroAPI\XeroPHP\Models\Accounting\CreditNote::STATUS_SUBMITTED)			
...
//var_dump($invoice);
		...
		$invoices = new XeroAPI\XeroPHP\Models\Accounting\CreditNotes;
		$invoices->setCreditNotes([$invoice]);




		$result = $apiInstance->createCreditNotes($xeroApi->getTenantId(), $invoices); // <---- ACTUAL POST REQUEST HERE
// echo "Here";
//var_dump($result);

		echo "Created CreditNotes: " . $result->getCreditNotes()[0]->getCreditNoteNumber() . ' --> '. $result->getCreditNotes()[0]->getTotal() . ' --> '. ltrim($lineArr[0]['document_number'],'0') . ' - ' . trim($lineArr[0]['customer_order_number']) . ' - ' . trim($lineArr[0]['branch_code']) . ' - ' . trim($lineArr[0]['deliver_name']) . ' - ' . trim($lineArr[0]['Area']). " \n";
