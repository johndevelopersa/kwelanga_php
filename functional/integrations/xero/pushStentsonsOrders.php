<?php
/*
 https://kwelangaonlinesolutions.co.za/systems/kwelanga_system/kwelanga_php/functional/integrations/xero/pushStentsonsOrders.php";
...
$principalId = 44;
$principalStoreSpecialFieldID = 606;	/* !!PENTACORP CATCH : Xero Contact ID!! */
/*-----------------------------------*/
...
$rTO = (new PostExtractDAO($dbConn))->queueAllInvoiced($principalId, 
                                                       $recipientUId,
...
// print_r($lineArr);
     ...
     strlen($storeSpecialArr[$storeId]['value']) != 36)	//xero ids are 36 chars
  {
	...
	$invoice->setType(XeroAPI\XeroPHP\Models\Accounting\Invoice::TYPE_ACCREC)
			->setReference(ltrim($lineArr[0]['document_number'],'0') . ' - ' . trim($lineArr[0]['customer_order_number']) . ' - ' . trim($lineArr[0]['depot_name']) . ' - ' . trim($lineArr[0]['deliver_name'])) 
...
// var_dump($result);
		...
		echo "Created Invoice: " . $result->getInvoices()[0]->getInvoiceNumber() . ' --> '. $result->getInvoices()[0]->getTotal() . ' - ' . ltrim($lineArr[0]['document_number'],'0') . ' - ' . trim($lineArr[0]['depot_name']) . ' - ' . trim($lineArr[0]['deliver_name']) . "\n";
