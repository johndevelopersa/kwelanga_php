<?php
/*
 https://kwelangaonlinesolutions.co.za/systems/kwelanga_system/kwelanga_php/functional/integrations/xero/pushStentsonsInvoices.php";
...
// print_r($lineArr);
     ...
     strlen($storeSpecialArr[$storeId]['value']) != 36)	//xero ids are 36 chars
  {
	...
	$invoice->setType(XeroAPI\XeroPHP\Models\Accounting\Invoice::TYPE_ACCREC)
			->setReference(ltrim($lineArr[0]['document_number'],'0') . ' - ' . trim($lineArr[0]['customer_order_number']) . ' - ' . trim($lineArr[0]['branch_code']) . ' - ' . trim($lineArr[0]['deliver_name'])) 
