<?php
/*
 https://kwelangaonlinesolutions.co.za/systems/kwelanga_system/kwelanga_php/functional/integrations/xero/pushTrigzInvoices.php";
...
// print_r($lineArr);
  ...
  foreach($lineArr as $lineRow) {
       ...
       $productTotalQty    = $lineRow['document_qty'] * $lineRow['items_per_case'] ;
  ...
  $trigBox = $transactionDAO->getPackagingBoxes($lineRow["dm_uid"]);
  foreach($trigBox as $row) { 
	
       //CREATE XERO LINE ITEM FOR PACKAGING BOXES
       $lineObj = new XeroAPI\XeroPHP\Models\Accounting\LineItem;
       $lineObj->setItemCode($row["old_code"])
       ->setDescription("Boxes")
       ->setQuantity((int)$row["BoxQty"]);
	...
	try {
      ...
      if(is_array($result->getInvoices()[0]->getValidationErrors()) && count($result->getInvoices()[0]->getValidationErrors())){
          ...
          foreach($result->getInvoices()[0]->getValidationErrors() as $error){
                 ...
                 if(strpos($error->getMessage(), "must be unique") > 1) {
                 	     $statusFlag = FLAG_STATUS_CLOSED;
                       $general2 = $lineArr[0]['document_number'];
                 } else {
                 	     $statusFlag = FLAG_ERRORTO_ERROR;
          ...
          $updateSmartE = new OmniExtractDAO($dbConn);
          $uSE = $updateSmartE->updateSmartEventDirectly($lineArr[0]['se_uid'], $general1, $general2, $statusFlag);

      } else {
         
          $xeroInvNum = $result->getInvoices()[0]->getInvoiceNumber();
          echo "Created Invoice: " . $result->getInvoices()[0]->getInvoiceNumber() . ' --> '. $result->getInvoices()[0]->getTotal() . ' --> '. ltrim($lineArr[0]['document_number'],'0') . ' - ' . trim($lineArr[0]['customer_order_number']) . ' - ' . trim($lineArr[0]['branch_code']) . ' - ' . trim($lineArr[0]['deliver_name']) . ' - ' . trim($lineArr[0]['Area']). " \n";
      		echo "<br><br><br>";
