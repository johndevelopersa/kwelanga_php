...
/*
 * ------------------------------------------------------
 * PROCESSOR TO PUSH INVOICES TO XERO.COM FOR KAROO CATCH
 * ------------------------------------------------------
 */
/*-----------------------------------
...
$principalId = 365;
$principalStoreSpecialFieldID = 486;	/* !!KAROO CATCH : Xero Contact ID!! */
/*-----------------------------------*/
...
foreach($bucketArr as $lineArr){
		...
		strlen($storeSpecialArr[$storeId]['value']) != 36)	//xero ids are 36 chars
	{
		...
		echo "empty chain xero id for chain: {$storeId}!\n";
	...
	foreach($lineArr as $lineRow){
		...
		$productTotalDiscountValue = (float)$lineRow['discount_value'];
