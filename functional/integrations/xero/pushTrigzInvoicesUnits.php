<?php
/*
 https://kwelangaonlinesolutions.co.za/systems/kwelanga_system/kwelanga_php/functional/integrations/xero/pushTrigzInvoicesUnits.php";
...
$rTO = (new PostExtractDAO($dbConn))->queueAllInvoiced($principalId, $recipientUId,
    ...
    $p_dtArr = array(DT_ORDINV_ZERO_PRICE),
    $p_wDSArr = false,
    $fromInvDate=false,
    $toInvDate=false,
    $chainUIdIn=false,
     ...
     strlen(trim($storeSpecialArr[$storeId]['value'])) != 36)	//xero ids are 36 chars
  {
	...
	foreach($lineArr as $lineRow) {
   	...
   	if(strlen($lineRow['product_code']) > 8) {
		...
		$productUnitAmount = round($lineRow['net_price'],2) ;	//this could vary across invoices (times and sessions)
		$productTotalQty   = $lineRow['document_qty'] ;
...
if (count($successSEUIdArr) > 0) {
    ...
    $bIResult = (new PostBIDAO($dbConn))->setSmartEventStatusBulk((implode(",", $successSEUIdArr)), "SUCCESS", $xeroInvNum);
